...
A second, weaker regularity runs from wet to dry, already visible in the precipitation term above. With snow fraction and clean skill both controlled, damage rises toward the dry, low-yield end of the attribute space: the partial Spearman correlation of damage is $+0.18$ with aridity, $-0.26$ with mean precipitation, $-0.25$ with runoff ratio, and $-0.28$ with mean specific discharge (all $p < 3\times10^{-5}$, $N = 531$). We deliberately do not read this as a second, drought-specific mechanism, for two reasons. First, the climate and flow-magnitude descriptors involved are mutually correlated ($|\rho| = 0.60$--$0.94$ among aridity, mean precipitation, runoff ratio, and mean discharge), and additionally conditioning on mean specific discharge flips the aridity partial from $+0.18$ to $-0.13$: in this design, climatic dryness cannot be separated from the small mean flows---and the correspondingly sensitive NSE scale---of dry-country rivers. Second, unlike snow, the dry end shows no distinctive attack signature (Section~\ref{sec:mechanism}). We therefore report the wet--dry gradient as a robust descriptive regularity and reserve the mechanistic claim for the snow regime.
...
Both diagnostics single out the snow regime. Along the wet--dry gradient of Section~\ref{sec:hydro} the attack does not change instrument: the precipitation share of the damage in the driest catchments (aridity above 1.5) is indistinguishable from that in the humid remainder (median 0.46 versus 0.46), although the share rises continuously with aridity within the non-snow population (partial Spearman $+0.36$, $p = 2\times10^{-3}$, $N = 71$), and we find no evidence of a differential flood-peak or low-flow signature at the dry end. Two limits bound these null results: this subset contains only seven dry non-snow basins, so only very large contrasts would be detectable; and low-flow-specific diagnostics are intrinsically weak in intermittent rivers, where an NSE-based damage measure assigns low-flow days negligible weight. Snow-influenced catchments remain the only regime in which the attack demonstrably re-aims its perturbation.
