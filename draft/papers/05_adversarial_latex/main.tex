...
We minimize $\|\boldsymbol{\delta}\|_2$ subject to $\mathrm{NSE}\bigl(f(\mathbf{x}+\boldsymbol{\delta}),\mathbf{y}\bigr) < 0$ via a Lagrangian relaxation with a weight that trades perturbation size against constraint satisfaction, solved by 200 iterations of gradient descent and a 5-step binary search over the weight (learning rate 0.01). The output for each catchment is the smallest $L_2$ forcing perturbation, in standardized units, that drives the test-period NSE below zero.
