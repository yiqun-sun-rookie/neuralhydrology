...
We employ five perturbation methods that span a spectrum of increasing optimization strength (Table~\ref{tab:methods}). All methods operate on the standardized forcing time series (each variable rescaled to zero mean and unit variance over the training period). The perturbation budget $\varepsilon$ is the maximum amount, in those standardized units, by which any single timestep of any forcing variable may be shifted relative to its clean value; Table~\ref{tab:epsilon} translates $\varepsilon$ back into physical units such as mm/day and $^\circ$C.

This design should be interpreted diagnostically rather than predictively. The random and adversarial perturbations here do not claim to reproduce how operational forcing errors are most likely to occur. They ask, instead, whether---within a specified uncertainty budget and plausibility constraints---structured input errors can be \emph{constructed} that materially degrade model performance. Their role is therefore analogous to that of a stress test on a hydraulic structure: they certify the absence of harmful failure modes within a budget, rather than predict the typical loading.
...
The Fast Gradient Sign Method asks a single question: ``in which direction would each component of the forcing tensor have to move to make the predicted streamflow worse, if I am only allowed one shot of size $\varepsilon$?'' Backpropagation through the trained model returns, at every timestep and for every input variable, the partial derivative of the loss with respect to that input value. The sign of this derivative indicates whether nudging the value upward or downward worsens the prediction; FGSM applies a single push of magnitude $\varepsilon$ in the unfavorable direction at every component simultaneously,
\begin{equation}
\mathbf{x}_{\mathrm{adv}} = \mathbf{x} + \varepsilon \cdot \mathrm{sign}\!\left(\nabla_{\mathbf{x}} \mathcal{L}(f(\mathbf{x}), \mathbf{y})\right),
\end{equation}
where $\mathcal{L}$ is the negative NSE loss, $f$ is the trained model, and $\mathbf{y}$ is the observed streamflow. FGSM is the cheapest possible gradient-based attack---one forward pass to evaluate the loss and one backward pass to obtain the gradient---and yields a deterministic perturbation per input sequence. In hydrological terms, FGSM constructs the worst forcing-error pattern that a linear model would predict at the clean operating point.

\subsubsection{Auto-PGD (Iterative Adversarial)}

FGSM implicitly linearizes the model around the clean forcing. For small budgets this linearization is accurate, and a single sign step nearly saturates the achievable damage; for larger budgets the model's response to forcing perturbations curves away from the linear approximation within the $\varepsilon$ neighborhood, and the worst direction picked at the clean input is no longer the worst direction within the budget. Auto-PGD \cite{Croce2020} addresses this by iterating: rather than one step of size $\varepsilon$, it takes many smaller steps, recomputes the gradient at each new candidate forcing tensor, and projects the result back into the budget whenever a component would otherwise leave it:
\begin{equation}
...
\end{equation}
Here $\Pi_{\mathcal{B}(\mathbf{x},\varepsilon)}$ denotes projection onto the $\varepsilon$-ball around the clean input---operationally, clipping any timestep/variable component that would exceed the budget back to the boundary---and $\alpha_t$ is an adaptive step size that starts at a fraction of $\varepsilon$ and shrinks as the attack converges. We use 50 iterations and one restart from a random initialization within the $\varepsilon$-ball; the restart protects against poor local optima at modest additional cost. In hydrological terms, Auto-PGD constructs the worst forcing-error pattern that the actual nonlinear model can be coaxed into producing within the uncertainty budget, rather than the worst pattern that a linearization of the model would predict.

\subsubsection{C\&W Regression Attack}

FGSM and Auto-PGD ask: ``how much can the model fail under a fixed perturbation budget?'' The Carlini-Wagner (C\&W) attack \cite{Carlini2017}, adapted here for regression, asks the dual question: ``what is the smallest forcing perturbation that suffices to push the model below an acceptable performance threshold?'' This shifts the analysis from an attack-magnitude axis to a per-catchment safety-margin axis. A catchment that requires a large $L_2$ perturbation before its NSE drops below zero has a comfortable margin against forcing error of any structure; a catchment that can be broken by a tiny $L_2$ perturbation is operationally fragile.

We minimize $\|\boldsymbol{\delta}\|_2$ subject to $\mathrm{NSE}\bigl(f(\mathbf{x}+\boldsymbol{\delta}),\mathbf{y}\bigr) < 0$ using a Lagrangian relaxation with a weight that trades perturbation size against constraint satisfaction, solved by 200 iterations of gradient descent and a 5-step binary search over the weight (learning rate 0.01). The output is, for each catchment, the smallest $L_2$ forcing perturbation in standardized units that drives the test-period NSE below zero.

\subsubsection{Causal Trigger Attack}

The causal trigger attack isolates a hydrologically motivated question: of the damage achievable under unrestricted iterative attack, how much can a perturbation extract while confining itself to the period most relevant for flood early warning? Operationally, the days immediately preceding a flood peak are when forecast skill matters most and when input data are most often degraded by sensor failure or telemetry latency. We identify flood peaks as local maxima of observed discharge that exceed the 90th percentile of the test-period flow, and restrict perturbations to a temporal window of 1, 3, 7, or 14 days before each identified peak. Within the window, perturbations are optimized iteratively for 100 steps using the same gradient-projection scheme as Auto-PGD; outside the window, the forcing remains clean. Comparing the four window lengths quantifies how much of the catchment's vulnerability is concentrated in the immediate pre-event period.
...
Experiments are organized into five blocks, each addressing a distinct robustness question across all 531 benchmark basins. The \textit{attack-comparison} block applies the five perturbation methods of Section~2.2 at four budget levels ($\varepsilon \in \{0.01, 0.05, 0.1, 0.2\}$) under the magnitude constraint and an untargeted loss that aggregates over all timesteps; this block isolates the effect of attack method choice. The \textit{constraint-ablation} block holds the attack fixed at Auto-PGD and varies the constraint level (magnitude, physical, statistical) across three budgets ($\varepsilon \in \{0.05, 0.1, 0.2\}$) to isolate how plausibility constraints reduce achievable damage. The \textit{targeted-attack} block applies Auto-PGD at $\varepsilon = 0.1$ under three loss targets: an untargeted aggregate loss, a flood-targeted loss that concentrates the optimization on high-flow periods, and a low-flow-targeted loss that concentrates on dry periods, allowing a direct comparison of high-flow versus low-flow vulnerability. The \textit{causal-trigger} block restricts perturbations to pre-flood windows of 1, 3, 7, and 14 days at $\varepsilon = 0.1$ to quantify how much vulnerability is concentrated in the operational forecasting window. Finally, the \textit{minimum-perturbation} block runs the C\&W attack on each basin to estimate per-basin safety margins.

All experiments were executed as chunked GPU jobs and written to an isolated 531-basin output tree. Results were merged and deduplicated, yielding 17{,}523 individual experiment records across 531 unique basin IDs.
...
Table~\ref{tab:attack} reports median $\Delta$NSE across the 514 finite basin metrics for each attack method at four perturbation budgets. Three observations structure the rest of this subsection: (i)~the dominant gap is between methods that exploit the trained model's gradient (FGSM, Auto-PGD) and methods that perturb the forcing without consulting the model (the three random baselines), not between iterative and single-step gradient methods; (ii)~the gap between single-step FGSM and iterative Auto-PGD grows monotonically with $\varepsilon$; and (iii)~the underlying $\Delta$NSE distribution is heavy-tailed, with the at-risk tail also widening with $\varepsilon$.
...
The hierarchy among the five methods at $\varepsilon = 0.1$---Auto-PGD followed by FGSM, with the three random baselines clustered near zero---admits a unified explanation. Random methods perturb each timestep and forcing variable independently and are exponentially unlikely to align, by chance, with the particular forcing-error pattern to which the trained model is most sensitive. FGSM consults the model's gradient at the clean operating point and so finds an aligned direction, but takes only one step; whenever the model's response to forcing perturbations bends within the $\varepsilon$ neighborhood, the direction picked at the clean input is no longer the direction of the worst attainable perturbation. Auto-PGD iteratively follows the bending response, exploiting nonlinearities that FGSM cannot reach.
...
Four findings emerge from this study. First, the choice of attack method exerts a decisive influence on the robustness conclusions one draws about a standard LSTM rainfall-runoff configuration: iterative adversarial perturbations reveal a median vulnerability 17.5 times greater than Gaussian random noise at $\varepsilon = 0.1$, and FGSM, although closer to Auto-PGD than the random baselines are, still systematically underestimates the worst case at larger budgets. Second, vulnerability is not a tail phenomenon---under Auto-PGD at $\varepsilon = 0.1$, 32.5\% of finite basins fall below NSE $= 0$ and 17.5\% experience severe degradation with $\Delta$NSE $< -1$. Third, statistically undetectable perturbations, those preserving the per-variable mean and standard deviation of the forcing, retain roughly half of the unconstrained adversarial damage at every budget tested, indicating that routine moment-based quality control cannot rule out harmful forcing patterns. Fourth, pre-event data quality matters disproportionately: perturbations confined to the fourteen days preceding a flood peak still produce a median $\Delta$NSE of $-0.085$, precisely in the operational forecasting window in which input data are most consequential and most often degraded.

Taken together, these findings establish that robustness assessment for standard hydrological deep-learning configurations benefits from multi-level stress testing that goes beyond random noise and single-step adversarial probes. We recommend iterative adversarial evaluation at observation-error-scale budgets as a standard diagnostic component of hydrological stress-testing workflows; broader architectural generalization, defense-strategy evaluation, and coupling adversarial bounds to calibrated forcing-error models remain natural follow-ups.
