...
Three random perturbation methods serve as baselines, each representing an idealized form of forcing error. For each method we draw 10 independent random samples per evaluation and retain the sample with the largest NSE drop, so that each baseline reflects the worst case identified by a light random search rather than a single arbitrary draw. \textit{Gaussian noise} samples i.i.d.\ values from $\mathcal{N}(0,\varepsilon^2)$ at every timestep and forcing variable, clipped to $[-\varepsilon,\varepsilon]$, and represents white sensor noise. \textit{Multiplicative precipitation bias} scales only the precipitation channel by a single random factor drawn from $\mathcal{U}(1-\varepsilon,1+\varepsilon)$, leaving the other forcing variables unchanged, and represents a miscalibrated rain gauge whose reading is uniformly off by a small fraction throughout the period. \textit{Temporally correlated noise} smooths an i.i.d.\ Gaussian draw with a 7-day moving average and rescales the result so that the maximum absolute deviation respects the $\varepsilon$ bound; this produces perturbations that vary slowly across consecutive days, for example a gridded forcing product that misrepresents the same weather system over its lifetime. None of the three uses gradient information from the model. Together they characterize the average-case sensitivity of the trained model to perturbations of the corresponding statistical structure, with the worst-of-10 selection providing a slightly conservative (i.e., harder-to-beat) baseline.
...
\end{equation}
Here $\Pi_{\mathcal{B}(\mathbf{x},\varepsilon)}$ denotes projection onto the $\varepsilon$-neighborhood around the clean input, implemented by clipping any timestep--variable component that exceeds the budget back to the boundary, and $\alpha_t$ is an adaptive step size that starts at $2\varepsilon$ and is halved at preset checkpoints, and additionally whenever three consecutive iterations fail to reduce the loss. We use 50 iterations starting from a single random initialization within the $\varepsilon$-neighborhood; the random start mitigates poor local optima at modest additional cost. Whereas FGSM corresponds to the worst pattern predicted by a linearization of the model, Auto-PGD identifies the worst forcing-error pattern attainable under the actual nonlinear model within the same budget.
...
The causal trigger attack quantifies how much of the unrestricted iterative damage is achievable by perturbations confined to the period most relevant for flood early warning. The days immediately preceding a flood peak are operationally consequential: forecast skill is most valuable in this window, and input data are most often degraded by sensor failure or telemetry latency. We identify flood peaks as observed discharge values exceeding the 95th percentile of the test-period flow, with nearby peaks clustered to a minimum separation of 14 days. Perturbations are restricted to a window of 1, 3, 7, or 14 days before each identified peak; outside the perturbation window the forcing remains unperturbed. Within the perturbation window, the perturbation is optimized for 100 iterations of Adam (learning rate 0.01) on the mean squared error between predicted and observed flow, evaluated over a separate 7-day post-peak window. Because the perturbation window and the evaluation window do not overlap in time, the attack must propagate corruption forward through the LSTM hidden state to influence post-peak predictions. Comparing the four pre-event window lengths quantifies the share of vulnerability concentrated in the immediate pre-event period.
...
The second level, the \textit{physical constraint}, additionally clips each forcing variable to a physically admissible range: non-negative precipitation, non-negative shortwave radiation, non-negative vapor pressure, and temperatures bounded to climatologically reasonable extremes, following \citeA{Yang2026}. This rules out patently unphysical forcing patterns while leaving the magnitude budget unchanged.
