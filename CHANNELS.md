...
| `kalmannet-tukf28` | 顺序学习探针：先学滤波噪声、冻结、再学水文参数（信赖域约束），对照同时学 | 2026-09-02 | 独立落点 `/data1/home/sunyiq/kalmannet_tukf28_20260902`；仅操作本通道和该目录，不碰 `~/neuralhydrology`、其他通道、其他实验或既有证据；禁止 `scancel -u`，只按本通道 job ID 取消 |
...
| `zhenjiang-update-budget-20260922` | 镇江共同预测模型固定后，滚动编码与可微分无迹卡尔曼滤波的100轮训练预算敏感性检验 | 2026-09-22 | 独立落点 `/data1/home/sunyiq/zhenjiang_update_budget_20260922_001`；只操作本通道和该新目录；2017—2022数据用于训练与选轮，禁止读取2023和2024数据，不碰既有实验、通道或作业；最多一个36小时训练作业。 |
...
| `kalmannet-wrr-hp-extension` | 《水资源研究》论文超参数扩展及入围配置的三随机种子比较 | 2026-09-02 | 原落点 `/data1/home/sunyiq/kalmannet_wrr_hp_extension_20260902` 现只读；2026-09-07 用户授权在新落点 `/data1/home/sunyiq/kalmannet_wrr_finalist_replication_20260907` 追加恰好6次验证集训练；只写本通道与新落点，不读取测试集，不改论文或正式模型，不碰其他通道/作业/旧实验，不自动重试或重新排队；⛔ 禁用账号级批量取消 |
| `id32-transformer-inductive-bias` | ID32 归纳偏置消融：诊断因果 Transformer 在降雨—径流上输给长短期记忆网络是缺累加—衰减归纳偏置还是欠正则，六个单因子臂 | 2026-09-02 | 由 Claude 会话占用；独立落点 `/data1/home/sunyiq/id32_transformer_inductive_bias_20260902`；只读 ID30 已审计的候选安全数据包与其基线配置；**不修改 ID30、ID31 的部署或结果**、不碰 `~/neuralhydrology` 的 Git 状态、不访问封存的 1989–1999 评估、不调用正式评分服务；⛔ 禁用账号级批量取消 |
| `id33-transformer-recipe-repair` | ID33 Transformer 配方修复：修掉四个只打 Transformer 不打长短期记忆网络的缺陷（初始化压扁嵌入层、注意力从均匀平均起步、权重衰减不对等、270 个位置只监督 1 个），五个 Transformer 臂 + 一个公平对照臂 | 2026-09-04 | 由 Claude 会话占用；独立落点 `/data1/home/sunyiq/id33_transformer_recipe_repair_20260904`；只读 ID30 已审计的数据包与其 D01/B01 基线配置；**不修改 ID30、ID31、ID32 的部署或结果**、不碰 `~/neuralhydrology` 的 Git 状态、不访问封存的 1989–1999 评估；⛔ 禁用账号级批量取消 |
| `attrswap-daily` | 日尺度线中国线三个实验共用通道：①中国可供属性替换（PREREG_20260901，7 臂，seq 1–10）②气象口径替换（PREREG_20260908，9 臂，seq 11–29）③美国侧全球降水产品对比第一段（PREREG_20260909，6 臂，seq 30–49）④**第二段·近代合同**（PREREG_20260911，8 参照 + 24 候选 = 32 臂，seq 51–78，用户 2026-09-11「approve 4 all」授权；2026-09-13 裁决封板）⑤**第三段·滑动日界窗 / 噪声零模型 / 60 轮对照**（PREREG_20260916，6 个 60 轮臂 + 4 窗口臂×3 + 3 噪声臂×3 + shift1×3 + 守卫/E4/P7，seq 82 起，用户 2026-09-16「按推荐」定稿、2026-09-17「go」放行提取与部署）：只读探测、隔离部署、训练、取回；①②③ 2026-09-10 封板、④ 2026-09-13 封板 | 2026-09-05（2026-09-11 补登②③④；2026-09-17 补登⑤） | 五个独立落点 `/data1/home/sunyiq/attr_swap_daily_2026_09`、`forcing_swap_daily_2026_09`、`precip_swap_daily_2026_09`、`precip_swap2_daily_2026_09`（四者封板只读；precip2 的 26 个 run 目录由⑤的 E4 作业整目录**复制**到新根 `runs_s2_copy/`，原目录不写）、**`precip_swap3_daily_2026_09`（⑤，可写）**；影子数据符号链接（precip2/precip3 的 daymet 直连 `~/neuralhydrology/data/camels_us/basin_mean_forcing/daymet`，流量/属性经 attr 落点）；归档代码 `git archive 1f9804e` 在 attr 落点（precip2/precip3 有副本）；不碰 `~/neuralhydrology` 的 git 状态、其他通道、其他实验或既有证据；作业日志写落点不写信箱；禁止 `scancel -u`，只按本通道 job ID 取消；①②③ 只发只读探测 |
