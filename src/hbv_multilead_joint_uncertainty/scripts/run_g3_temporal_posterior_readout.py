"""Run the preregistered time-robust posterior-readout confirmation."""
...
from hbv_multilead_joint_uncertainty.temporal_posterior_readout import (  # noqa: E402
...
_TEMPORAL_READOUT = {
    ...
    "three_and_seven_day_rule": "highest_arithmetic_mean_posterior",
}
_COMPARISON_METHODS = (
    "temporal_posterior",
    "full_posterior",
...
_SOURCE_FILES = (
    "src/hbv_multilead_joint_uncertainty/temporal_posterior_readout.py",
    "src/hbv_multilead_joint_uncertainty/interaction_value_comparison.py",
    (
        "src/hbv_multilead_joint_uncertainty/scripts/"
        "run_g3_temporal_posterior_readout.py"
    ),
    (
        "src/hbv_multilead_joint_uncertainty/scripts/"
        "verify_g3_temporal_posterior_readout.py"
    ),
    (
    ...
    "test/test_hbv_temporal_posterior_readout.py",
    "test/test_hbv_temporal_posterior_readout_config_contract.py",
    "test/test_hbv_temporal_posterior_readout_runner.py",
    "test/test_hbv_temporal_posterior_readout_verifier.py",
)
...
def _validate_config_contract(config: dict) -> None:
    ...
    if config.get("temporal_readout") != _TEMPORAL_READOUT:
        raise ValueError("temporal readout does not match the frozen contract")
...
def _generated_blocks(config: dict) -> tuple[dict[str, int | str], ...]:
    """Expand the frozen compact seed-family contract."""
    ...
    if isinstance(count, bool) or not isinstance(count, (int, np.integer)):
        raise ValueError("matched block count must be an integer")
    count = int(count)
    if count <= 0:
        raise ValueError("matched block count must be positive")
...
def _with_generated_blocks(
    ...
    """Adapt the compact seed contract to shared resource accounting."""
...
def _derive_confirmation(result, driver: dict, config: dict) -> dict:
    """Derive the frozen temporal readout from stored probability histories."""
    ...
    probability_history = np.asarray(
    ...
    temporal_probabilities = np.empty(
        (block_count, truth_count, candidate_count),
        dtype=np.float64,
    )
    selected_indices = np.empty((block_count, truth_count), dtype=np.int64)
    window_days = int(config["temporal_readout"]["window_days"])
    for block in range(block_count):
        for truth in range(truth_count):
            readout = temporal_posterior_readout(
                probability_history[block, truth],
            ...
            temporal_probabilities[block, truth] = (
                readout.temporal_mean_probabilities
            )
    ...
    one_day_difference = float(
        np.max(
            np.abs(readout_forecasts[..., 0] - full_forecasts[..., 0]),
            initial=0.0,
        )
    )
    ...
    forecasts = {
        "temporal_posterior": readout_forecasts,
    ...
    statistics = summarize_temporal_posterior_readout(
        forecasts=forecasts,
    ...
    expected_selected = np.argmax(
        np.mean(probability_history[:, :, -window_days:], axis=2),
        axis=-1,
    )
    ...
    cross_checks = {
        ...
        "readout_reconstruction_maximum_absolute_error": (
            readout_reconstruction_error
        ),
        "one_day_maximum_absolute_difference": one_day_difference,
        "selected_candidate_indices_match_temporal_argmax": bool(
            selection_matches
        ),
    ...
    return {
        ...
        "full_probability_history": probability_history,
        "full_final_probabilities": final_probabilities,
        "temporal_weights": readout_weights,
        "temporal_mean_probabilities": temporal_probabilities,
        "selected_candidate_indices": selected_indices,
        "true_candidate_indices": true_candidate_indices,
...
def _result_summary(statistics: dict) -> dict:
    ...
    return {
        ...
        "one_day_maximum_absolute_difference": float(
            statistics["one_day_maximum_absolute_difference"]
        ),
        "retention_gates": {
            key: bool(value)
...
def _evidence_arrays(
    ...
    arrays: dict[str, np.ndarray] = {
        ...
        "full_probability_history": derived["full_probability_history"],
        "full_final_probabilities": derived["full_final_probabilities"],
        "none_posterior_forecasts": derived["forecasts"]["none_posterior"],
        "temporal_posterior_forecasts": derived["forecasts"][
            "temporal_posterior"
        ],
        "temporal_weights": derived["temporal_weights"],
        "temporal_mean_probabilities": derived[
...
def run(repo_root: Path, config_path: Path, output_dir: Path) -> dict:
    ...
    preregistration = {
        ...
        "matched_block_generation": config[
