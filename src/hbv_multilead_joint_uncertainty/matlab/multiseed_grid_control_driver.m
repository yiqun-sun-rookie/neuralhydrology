function multiseed_grid_control_driver(seeds, out_dir)
% Grid-candidate control: identical to multiseed_params_driver except that
% candidates are the deterministic published-workspace linspace grid.
...
% RNG stream consists of the noise draws only (no lhsdesign, no burned draws).
...
% minimal set (imm_probs, candidates, qobs).
    ...
    for seed = seeds(:)'
        ...
        try
            run_one_seed_grid(seed, out_dir, qobs_base, p, etpot, cd, cs);
...
function run_one_seed_grid(seed, out_dir, qobs, p, etpot, cd, cs)
    rng(seed, "twister");
    ...
    bounds = [700 300; % cw
              ...
              1.9e7 1.5e7; % cg
              7 2]; % cq
    cw = linspace(bounds(1, 2), bounds(1, 1), n)';
    cv = linspace(bounds(2, 2), bounds(2, 1), n)';
    cg = linspace(bounds(3, 2), bounds(3, 1), n)';
    cq = linspace(bounds(4, 2), bounds(4, 1), n)';
    cw = [cw; cw_truth(1:2)'];
    cv = [cv; cv_truth(1:2)'];
    cg = [cg; cg_truth(1:2)'];
    cq = [cq; cq_truth(1:2)'];
