"""Same-day global-posterior state audit for interaction versus no interaction.

Both methods contain the same three fixed parameter models and both publish one
posterior-probability-weighted global posterior after every update.  The only
changed factor is whether model-conditioned states and covariances interact
before filtering.  No forecast operation belongs in this module.
"""
...
def summarize_state_interaction_global_posterior_error(
    method_states,
    truth_states,
    ...
    """Compare daily full and no-interaction global posteriors with truth."""
    ...
    if methods != INTERACTION_STATE_METHODS or method_count != 2:
        raise ValueError("method order differs from the interaction-only contract")
    ...
    per_state_standardized_rmse = (
        per_state_rmse / truth_standard_deviation[None, :]
    )
    per_state_block_mse = np.mean(squared, axis=(1, 3))
    per_state_block_mse_difference = (
        per_state_block_mse[:, 0] - per_state_block_mse[:, 1]
    )
    ...
    group_block_difference = (
        group_block_standardized_mse[:, 0]
        - group_block_standardized_mse[:, 1]
    )
    group_bootstrap_difference = np.mean(
        group_block_difference[bootstrap], axis=1
    )
    ...
    return {
        ...
        "per_state_relative_rmse_fraction": per_state_rmse[0] / per_state_rmse[1]
        - 1.0,
        ...
        "group_relative_standardized_rmse_fraction": (
            group_standardized_rmse[0] / group_standardized_rmse[1] - 1.0
        ),
        ...
        "complete_relative_standardized_rmse_fraction": float(
            complete_standardized_rmse[0] / complete_standardized_rmse[1] - 1.0
        ),
        "complete_decision": str(
            _decision(complete_ci_low, complete_ci_high).item()
        ),
