...
输出:
    - data/test_data/ 目录，包�?个流域的完整数据
    - src/test_data/configs/ 目录，包含测试配置文�?"""
...
def create_test_data():
    ...
    config_dir = Path("src/test_data/configs")
    ...
    for basin in test_basins:
        ...
        if src_file.exists():
            ...
            print(f"  警告: 未找到径流数�?{basin}")
    
    # 创建basin_mean_forcing目录并复制气象数�?
    print("复制气象数据...")
    ...
    print(f"  创建: test_train_basins.txt ({len(train_basins)} 个流�?")
    print(f"  创建: test_val_basins.txt ({len(val_basins)} 个流�?")
    print(f"  创建: test_test_basins.txt ({len(test_basins)} 个流�?")
    
    # 创建测试配置文件
    print("创建测试配置文件...")
    create_test_configs(config_dir, test_data_dir, test_basins)
    ...
    print(f"[INFO] 估计大小: ~50MB (相比完整数据�?~10GB)")

def _build_config_content(experiment_name, epochs, config_dir, test_data_dir, test_basins):
    """构建测试配置文件内容"""
    return f"""# 轻量级测试配�?# 用于快速验证neuralhydrology库的安装和运�?# 包含3个流域，训练时间�?-10分钟
...
# 训练配置（快速训练）
epochs: {epochs}  # 较少的训练轮�?batch_size: 64  # 较小的批次大�?
optimizer: Adam
loss: MSE
...
def create_test_configs(config_dir, test_data_dir, test_basins):
    """创建 quick_test �?test_config 两套测试配置"""
    quick_content = _build_config_content(
    ...
    with open(config_dir / "quick_test.yml", "w", encoding="utf-8") as f:
        f.write(quick_content)
    print("  创建: quick_test.yml")

    standard_content = _build_config_content(
