...
使用方式：
    # 检查所有训练运行状态
    python src/full_531_basins/scripts/training_recovery.py --check-all
    
    # 诊断特定训练运行
    python src/full_531_basins/scripts/training_recovery.py --diagnose --run-dir runs/full_training_2025_1024_1631
    
    # 自动恢复失败的训练
    python src/full_531_basins/scripts/training_recovery.py --auto-recover
    
    # 验证数据完整性
    python src/full_531_basins/scripts/training_recovery.py --verify-data
...
class TrainingRecovery:
    ...
    def _attempt_recovery(self, run_dir):
        ...
        try:
            ...
            # 尝试恢复训练
            cmd = [
                sys.executable, "src/full_531_basins/scripts/full_train.py",
                "--resume",
                "--run-dir", str(run_dir),
                "--gpu", "0"
            ]
