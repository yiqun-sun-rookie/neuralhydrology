...
setup_environment() {
    ...
    # 运行环境设置脚本
    if [ -f "$PROJECT_DIR/src/full_531_basins/hpc/setup_hpc_environment.sh" ]; then
        print_info "运行环境设置脚本..."
        bash "$PROJECT_DIR/src/full_531_basins/hpc/setup_hpc_environment.sh"
    else
        print_error "环境设置脚本不存在: $PROJECT_DIR/src/full_531_basins/hpc/setup_hpc_environment.sh"
        exit 1
    fi
...
run_quick_test() {
    ...
    # 检查配置文件
    local config_file="$PROJECT_DIR/src/full_531_basins/configs/camels_us/hpc/hpc_quick_test.yml"
    ...
    # 检查SLURM脚本
    local slurm_script="$PROJECT_DIR/src/full_531_basins/hpc/hpc_quick_test.slurm"
...
run_full_training() {
    ...
    # 检查配置文件
    local config_file="$PROJECT_DIR/src/full_531_basins/configs/camels_us/hpc/hpc_full_training.yml"
    if [ ! -f "$config_file" ]; then
    ...
    # 检查SLURM脚本
    local slurm_script="$PROJECT_DIR/src/full_531_basins/hpc/hpc_full_training.slurm"
    if [ ! -f "$slurm_script" ]; then
...
monitor_training() {
    ...
    # 检查监控脚本
    local monitor_script="$PROJECT_DIR/src/full_531_basins/scripts/monitor_training.py"
