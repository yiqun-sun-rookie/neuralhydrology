...
ssh ${User}@${HostName} "mkdir -p ~/neuralhydrology/src/41_mts_mamba_global_transfer ~/neuralhydrology/logs/41_mts_mamba_global_transfer ~/neuralhydrology/results/41_mts_mamba_global_transfer"
if ($LASTEXITCODE -ne 0) {
  Write-Host "Failed to create remote directories." -ForegroundColor Red
  exit 1
}

scp -r src/41_mts_mamba_global_transfer ${User}@${HostName}:${RemoteProjectPath}src/
