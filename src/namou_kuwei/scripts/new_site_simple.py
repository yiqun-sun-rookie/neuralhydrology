...
用法：
    python src/namou_kuwei/scripts/new_site_simple.py <站点名> <数据目录>
    
示例：
    python src/namou_kuwei/scripts/new_site_simple.py mekong_vientiane F:/data/mekong_hourly
...
TEMPLATE_DIR = Path(__file__).resolve().parents[1] / "configs" / "no_leak"

def create_site(site_name: str, data_dir: str):
    """创建新站点目录结构"""
    
    project_root = Path(__file__).resolve().parents[3]
    site_dir = project_root / "src" / site_name / "configs"
    ...
    for tpl in templates:
        ...
        content = content.replace(
            "F:/github/pycharm/projects/neuralhydrology/data/namou_kuwei/hourly",
            data_dir.replace("\\", "/")
        )
...
2. 检查数据: python src/namou_kuwei/scripts/check_site.py {site_name}
