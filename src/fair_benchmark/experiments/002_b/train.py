"""Train ONE global XGBoost rainfall-runoff regressor for Track-0 (002_b).
...
(with a one-year warmup so the long rolling / recursive features are valid),
engineers the SAME enriched features predict.py will use (via
submission/features.py: 001 base + exponential API memory + degree-day snowpack
+ radiation PET / water-balance), pools every basin into one table, and fits a
single XGBoost on log1p(Q). Basins are distinguished only by their 27 static
attributes -> a genuinely global model.

Family vs 001: gradient boosting with histogram trees, XGBoost instead of
LightGBM, plus the physically motivated catchment-memory / snow feature block.

Saves model/xgb.json + model/features.json (the exact feature spec predict.py
reloads). Run via the spine:
    python -m fair_benchmark.run_candidate --exp src/fair_benchmark/experiments/002_b
"""
import json
import sys
import time
...
def main():
    ...
    spec = {
        ...
        "api_decays": F.API_DECAYS,
        "liquid_sum_windows": F.LIQUID_SUM_WINDOWS,
        "cwb_windows": F.CWB_WINDOWS,
