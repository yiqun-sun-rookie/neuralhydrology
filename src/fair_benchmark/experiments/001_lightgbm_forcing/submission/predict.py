"""001 LightGBM challenger — SUBMISSION side (scanned for leakage).
...
features used at train time (shared submission/features.py), runs the global
LightGBM, inverts the log1p target (expm1), clips negatives to 0, and writes a
long CSV (basin,date,qsim in mm/day) over the eval window for all basins.
...
def predict(out_csv):
    ...
    for basin, g in forcing.groupby("basin", sort=True):
        ...
        # long rolling windows are valid, then keep only the eval rows.
