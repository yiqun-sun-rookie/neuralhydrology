"""Causal extraction of the version 03 classic and older-history input bands."""
...
def fixed_band_specs_v03() -> tuple[BandSpec, ...]:
    """Return the frozen, disjoint 3650-day version 03 input bands."""
    return (
...
def lag_bin_edges(spec: BandSpec) -> np.ndarray:
    """Return integer edges that cover every lag in ``spec`` exactly once."""
...
def forcing_prefix(x: torch.Tensor) -> torch.Tensor:
    """Build a cumulative forcing tensor for repeated pooled extraction."""
    if x.ndim != 3:
...
def _validate_indices(
    ...
    required_history = max(spec.end_lag for spec in fixed_band_specs_v03())
...
def gather_fixed_bands_v03(
    x: torch.Tensor,
    basin_indices: torch.Tensor,
    target_indices: torch.Tensor,
    prefix: torch.Tensor | None = None,
) -> dict[str, torch.Tensor]:
    """Extract recent daily and older pooled inputs in chronological order."""
