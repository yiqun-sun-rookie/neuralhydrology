...
from models_v04 import VARIANTS_V04
import train_v04
...
def _config() -> dict:
    return {
        ...
        "reference_summary_sha256": "d" * 64,
    }


def test_v04_learning_rate_schedule_matches_classic_training():
    config = _config()
    expected = {
        1: 0.001,
        11: 0.001,
    ...
    assert {epoch: train_v04.learning_rate_for_epoch(config, epoch) for epoch in expected} == expected


def test_v04_dynamic_batch_exposes_only_inputs_consumed_by_each_arm():
    forcing = train_v04.torch.zeros((1, 4000, 5))
    prefix = train_v04.forcing_prefix(forcing)
    basins = train_v04.torch.tensor([0])
    targets = train_v04.torch.tensor([3649])

    classics = ("classic_lstm_256", "classic_lstm_261", "classic_lstm_265", "classic_lstm_270")
    for variant in classics:
        dynamic = train_v04.dynamic_batch_v04(variant, forcing, prefix, basins, targets)
...
@pytest.mark.parametrize("variant", VARIANTS_V04)
def test_v04_tiny_run_writes_atomic_recomputable_artifacts(tmp_path, variant):
    output_dir = tmp_path / f"{variant}_s100"
    manifest = train_v04.run_experiment_v04(
...
def test_v04_runner_rejects_unknown_variant(tmp_path):
...
def test_v04_frozen_config_rejects_architecture_or_training_drift(key, bad_value):
    config = _config()
    config[key] = bad_value
    with pytest.raises(ValueError, match=key):
        train_v04.validate_frozen_config_v04(config)


@pytest.mark.parametrize(
    "missing_key",
    ["design_commit", "basin_file_sha256", "target_bundle_sha256", "reference_summary_sha256"],
)
def test_v04_frozen_config_requires_evidence_bindings(missing_key):
    config = _config()
...
def test_v04_pilot_requires_thirty_epochs_and_unlimited_batches():
    config = _config()
...
def test_v04_frozen_file_hash_validation_rejects_changed_file(tmp_path):
    path = tmp_path / "basins.txt"
    path.write_text("00000001\n", encoding="utf-8")
    expected = hashlib.sha256(path.read_bytes()).hexdigest()
    assert train_v04.validate_frozen_file_hash(path, expected, "basin list") == expected

    path.write_text("00000002\n", encoding="utf-8")
    with pytest.raises(ValueError, match="basin list SHA-256"):
        train_v04.validate_frozen_file_hash(path, expected, "basin list")


def test_v04_config_snapshot_records_variant_and_seed(tmp_path):
    output_dir = tmp_path / "classic_lstm_256_s100"
    train_v04.run_experiment_v04(
        pack=_synthetic_pack(),
        config=_config(),
...
@pytest.mark.parametrize(("name", "mode"), [("smoke_v04.json", "smoke"), ("pilot_v04.json", "pilot")])
def test_v04_tracked_configs_are_valid_and_bound(name, mode):
    config_path = Path(__file__).resolve().parents[1] / "configs" / name
    config = json.loads(config_path.read_text(encoding="utf-8"))
    train_v04.validate_frozen_config_v04(config)
    assert config["mode"] == mode
    assert config["reference_summary_sha256"] == (
