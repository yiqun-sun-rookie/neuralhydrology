"""Atomic runner for persistent and recent-conditioned historical context."""
...
from models_v04 import VARIANTS_V04, build_model_v04, count_trainable_parameters
...
def validate_frozen_config_v04(config: Mapping) -> None:
    """Reject any version 04 architecture, training, or evidence-binding drift."""
...
def run_experiment_v04(
    ...
    """Train one version 04 arm and write independently recomputable evidence."""
    if variant not in VARIANTS_V04:
        raise ValueError(f"variant must be one of {VARIANTS_V04}")
    ...
    model = build_model_v04(variant, seed=seed).to(torch_device)
    ...
    manifest = {
        ...
        "reference_summary_sha256": config["reference_summary_sha256"],
        "variant": variant,
        "seed": int(seed),
        "parameter_count": parameter_count,
...
def main() -> None:
    ...
    parser.add_argument("--variant", choices=VARIANTS_V04, required=True)
    ...
    reference_summary = _REPO / config["reference_results_root"] / "summary.json"
