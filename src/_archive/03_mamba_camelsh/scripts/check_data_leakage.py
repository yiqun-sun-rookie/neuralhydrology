...
def main():
    ...
    if target in dynamic_inputs:
        print('[WARN] 警告: 目标变量出现在输入中!')
    else:
        print('[OK] 目标变量未出现在动态输入中')
    ...
    print('地形属性 (不变): ', topo_attrs, '[OK]')
    print('土壤属性 (不变): ', soil_attrs, '[OK]')
    print('植被属性 (缓变): ', veg_attrs, '[OK]')
    print('气候属性 (需检查): ', climate_attrs, '[WARN]')
