...
# 
# 用法:
#   ./common/submit.sh <task_name> [slurm_script] [extra_sbatch_args]
#
# 示例:
#   ./common/submit.sh mamba_camels_us                    # 全量 Mamba 训练
#   ./common/submit.sh mamba_quick                        # Mamba 快速验证
#   ./common/submit.sh caravan                            # Caravan 训练
#   ./common/submit.sh lstm_benchmark common/custom.slurm # 自定义脚本
#
# 日志结构:
#   logs/
#   ├── mamba_camels_us/
...
declare -A TASK_SCRIPTS=(
    ...
    ["caravan"]="src/caravan_global/hpc/submit_caravan.slurm"
...
show_help() {
    ...
    echo -e "==========================================${NC}"
    echo ""
    echo "用法: ./common/submit.sh <task_name> [slurm_script] [extra_args]"
    echo ""
    ...
    echo ""
    echo -e "${YELLOW}示例:${NC}"
    echo "  ./common/submit.sh mamba_quick                 # 快速验证"
    echo "  ./common/submit.sh mamba_camels_us             # 全量训练"
    echo "  ./common/submit.sh caravan                     # Caravan 训练"
    echo "  ./common/submit.sh my_exp common/custom.slurm  # 自定义脚本"
    echo ""
...
if [ -z "$SLURM_SCRIPT" ]; then
    ...
    if [ -n "${TASK_SCRIPTS[$TASK_NAME]}" ]; then
        ...
        echo "请指定 SLURM 脚本: ./common/submit.sh $TASK_NAME <slurm_script>"
        exit 1
    fi
fi
