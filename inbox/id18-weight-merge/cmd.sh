...
JOB_FILE=${LANDING}/A800_RETRY_NO_PYTEST_JOB_ID.txt
[ -f "${JOB_FILE}" ] || { echo "NO_PYTEST_JOB_FILE_MISSING"; exit 1; }
JOB_ID=$(cat "${JOB_FILE}")
...
if [ -f "${STDOUT}" ]; then
    echo "=== STDOUT TAIL ==="
    tail -n 120 "${STDOUT}" || true
fi
if [ -s "${STDERR}" ]; then
    echo "=== STDERR TAIL ==="
    tail -n 120 "${STDERR}" || true
fi
...
echo "=== TM01 A800 FINAL STATUS COMPLETE ==="
exit 0
