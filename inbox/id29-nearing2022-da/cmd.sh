...
sacct -X -n -P -S $(date -d '10 days ago' +%Y-%m-%d) --format=JobID,JobName,State,ExitCode,Elapsed,End 2>/dev/null | grep -E 'N22' | grep -E '\|(TIMEOUT|FAILED|NODE_FAIL|OUT_OF_MEMORY|CANCELLED)' || echo '  none'
echo "=== WARMPAIR 219423 STATE ==="
sacct -j 219423 -X -n -P --format=JobID,State,Elapsed,End 2>/dev/null || echo '  not found'
