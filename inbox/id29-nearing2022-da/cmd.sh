...
echo "=== WARMPAIR ERR TAILS ==="
ls -t "$ROOT"/logs/29_nearing2022_da_ar/*219423* 2>/dev/null | head -4 || true
for F in $(ls -t "$ROOT"/logs/29_nearing2022_da_ar/*219423*.err 2>/dev/null | head -2); do echo "--- $F"; tail -n 15 "$F" || true; done
echo "=== ARTIFACTS ==="
