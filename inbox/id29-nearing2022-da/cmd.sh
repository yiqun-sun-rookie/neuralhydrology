#!/bin/bash
# ID29 seq=273: independently rehash and closure-enumerator-cover all 112 seq=271 audit sources.
set -eo pipefail

ROOT=/data1/home/sunyiq/nearing2022_da
IDEA="$ROOT/src/29_nearing2022_da_ar"
AUDIT="$ROOT/results/29_nearing2022_da_ar/formal_closure/diagnostics/partial_numerical_audit_seq271_v1/audit_1.json"
OLD_AUDIT="$ROOT/results/29_nearing2022_da_ar/formal_closure/diagnostics/partial_numerical_audit_seq249_v1/audit_1.json"
ROLE="$ROOT/results/29_nearing2022_da_ar/formal_closure/diagnostics/partial_numerical_audit_seq271_v1/role_snapshot.json"
...
test "$(stat -c '%s' "$AUDIT")" = 82615
test "$(sha256sum "$AUDIT" | awk '{print $1}')" = 13ee5257cc949513df9e69d5fe8e60520611be1661416f170d7404572fd60b6f
test "$(stat -c '%s' "$OLD_AUDIT")" = 78876
test "$(sha256sum "$OLD_AUDIT" | awk '{print $1}')" = 49fe6df436f663bea770ec08d23926d4acf44bdeec5791f06f3c2af6c80b2cb7
test "$(stat -c '%s' "$ROLE")" = 2934
test "$(sha256sum "$ROLE" | awk '{print $1}')" = 8e7c8930b259890bbdbfedef9143747a67690cc9f8dc510e8bdcc234bd3c143b
...
if audit['complete_coordinates'] != 21 or audit['comparison_rows'] != 147:
    raise ValueError('Unexpected current audit dimensions')
if old_audit['complete_coordinates'] != 20 or old_audit['comparison_rows'] != 140:
...
if role['slurm_job_id'] != '202778' or role['slurm_node'] != 'ngu102':
    raise ValueError('Role snapshot execution identity differs')
if role['partial_complete_count'] != 21 or role['closure_complete_count'] != 21:
    raise ValueError('Role snapshot count differs')
if role['target'] != 'N22-EVAL-TS-DA-L08-TE000-S0':
...
if len(artifacts) != 112 or len(old_artifacts) != 107:
...
if verified_bytes != 1066375781 or len(verified_hashes) != 91:
...
expected_added_coordinates = ['N22-EVAL-TS-DA-L08-TE000-S0']
...
if len(added_sources) != 5 or added_source_bytes != 48319505:
    raise ValueError(
        f'Unexpected added sources: count={len(added_sources)}, bytes={added_source_bytes}'
    )
print(json.dumps({
    'schema': 'nearing2022-partial-source-rehash-and-final-manifest-coverage-v7',
