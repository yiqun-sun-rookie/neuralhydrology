#!/bin/bash
# ID29 seq=277: independently rehash and closure-enumerator-cover all 117 seq=275 audit sources.
set -eo pipefail

ROOT=/data1/home/sunyiq/nearing2022_da
IDEA="$ROOT/src/29_nearing2022_da_ar"
AUDIT="$ROOT/results/29_nearing2022_da_ar/formal_closure/diagnostics/partial_numerical_audit_seq275_v1/audit_1.json"
OLD_AUDIT="$ROOT/results/29_nearing2022_da_ar/formal_closure/diagnostics/partial_numerical_audit_seq271_v1/audit_1.json"
ROLE="$ROOT/results/29_nearing2022_da_ar/formal_closure/diagnostics/partial_numerical_audit_seq275_v1/role_snapshot.json"
...
test "$(stat -c '%s' "$AUDIT")" = 86336
test "$(sha256sum "$AUDIT" | awk '{print $1}')" = caf456671aee9b1b4b274793911d85d3d3c242cd4344392506fb608c9c29a3cf
test "$(stat -c '%s' "$OLD_AUDIT")" = 82615
test "$(sha256sum "$OLD_AUDIT" | awk '{print $1}')" = 13ee5257cc949513df9e69d5fe8e60520611be1661416f170d7404572fd60b6f
test "$(stat -c '%s' "$ROLE")" = 2934
test "$(sha256sum "$ROLE" | awk '{print $1}')" = 50ab4d874628c469d1a73eb9e5bfcd4ee75a1e9a381d10dbadfeb83236237b49
...
if audit['complete_coordinates'] != 22 or audit['comparison_rows'] != 154:
    raise ValueError('Unexpected current audit dimensions')
if old_audit['complete_coordinates'] != 21 or old_audit['comparison_rows'] != 147:
...
if role['slurm_job_id'] != '202780' or role['slurm_node'] != 'ngu101':
    raise ValueError('Role snapshot execution identity differs')
if role['partial_complete_count'] != 22 or role['closure_complete_count'] != 22:
    raise ValueError('Role snapshot count differs')
if role['target'] != 'N22-EVAL-TS-DA-L08-TE025-S0':
...
if len(artifacts) != 117 or len(old_artifacts) != 112:
...
if verified_bytes != 1114693076 or len(verified_hashes) != 95:
...
expected_added_coordinates = ['N22-EVAL-TS-DA-L08-TE025-S0']
...
if len(added_sources) != 5 or added_source_bytes != 48317295:
    raise ValueError(
        f'Unexpected added sources: count={len(added_sources)}, bytes={added_source_bytes}'
    )
print(json.dumps({
    'schema': 'nearing2022-partial-source-rehash-and-final-manifest-coverage-v8',
