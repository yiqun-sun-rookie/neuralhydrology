...
echo "=== WARMPAIR JOBS (14d, any name) ==="
sacct -X -n -P -S $(date -d '14 days ago' +%Y-%m-%d) --format=JobID,JobName,State,ExitCode,Elapsed,End 2>/dev/null | grep -Ei 'warm' || echo '  none'
echo "=== WARMPAIR DIRS ==="
...
echo "=== PREPARE SCRIPT L94 + RECEIPT JOB ID ==="
