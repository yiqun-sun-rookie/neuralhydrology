...
for J in 219423_0 219423_1 220487; do
  echo "=== $J ==="
  sacct -j "$J" -n -P --format=JobID,JobName,State,WorkDir 2>/dev/null | head -2 || true
done
...
echo "=== TAILS ==="
for F in $(find "$ROOT" -maxdepth 5 -newermt '2026-09-03' \( -name '*219423*' -o -name '*220487*' \) 2>/dev/null | head -8); do
  echo "--- $F"
