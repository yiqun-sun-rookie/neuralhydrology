#!/bin/bash
# ID29 seq=276: verify and recover the 22-coordinate isolated audit when terminal.
set -eo pipefail

ROOT=/data1/home/sunyiq/nearing2022_da
JOB_ID=202780
JOB_NAME=N22-part-audit11
TARGET=N22-EVAL-TS-DA-L08-TE025-S0
FINAL="$ROOT/results/29_nearing2022_da_ar/formal_closure/diagnostics/partial_numerical_audit_seq275_v1"
PREVIOUS="$ROOT/results/29_nearing2022_da_ar/formal_closure/diagnostics/partial_numerical_audit_seq271_v1"
...
if test "$STATE" != "COMPLETED" || test "$EXIT_CODE" != "0:0"; then
  ...
  for staging in "$(dirname "$FINAL")"/partial_numerical_audit_seq275_v1.preparing-*; do
...
if audit["complete_coordinates"] != 22:
    raise ValueError("Terminal audit does not contain exactly 22 complete coordinates")
if audit["comparison_rows"] != 154:
    raise ValueError("Terminal audit does not contain exactly 154 metric comparisons")
...
if node == "ngu104" or role["slurm_node"] == "ngu104":
    raise ValueError("Excluded node ngu104 executed the audit")
...
if role["partial_complete_count"] != 22 or role["partial_complete_count"] != audit["complete_coordinates"]:
...
if len(old_coordinates) != 21 or len(new_coordinates) != 22:
    raise ValueError("Predecessor or current coordinate count differs from the frozen incremental contract")
...
if added != [target]:
    raise ValueError(f"The single newly complete target differs: {added}")
...
if receipt["mailbox_sequence"] != 275:
...
summary = {
    "schema": "nearing2022-partial-numerical-audit-seq275-terminal-verification-v1",
    "job_id": job_id,
    "slurm_node": node,
