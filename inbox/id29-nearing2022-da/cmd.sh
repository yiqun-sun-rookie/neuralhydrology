#!/bin/bash
# ID29 seq=225: independently rehash and final-manifest-cover all 92 seq=223 audit sources.
set -eo pipefail

ROOT=/data1/home/sunyiq/nearing2022_da
IDEA="$ROOT/src/29_nearing2022_da_ar"
AUDIT="$ROOT/results/29_nearing2022_da_ar/formal_closure/diagnostics/partial_numerical_audit_seq223_v1/audit_1.json"
OLD_AUDIT="$ROOT/results/29_nearing2022_da_ar/formal_closure/diagnostics/partial_numerical_audit_seq205_v1/audit_1.json"
...
test ! -L "$OLD_AUDIT"
test ! -L "$SCRIPT"
test "$(stat -c '%s' "$AUDIT")" = 67714
test "$(sha256sum "$AUDIT" | awk '{print $1}')" = 842b03b46c6f80f35310fdab0173894d6e5a430292a0d0478c0c0e7b52043dcc
test "$(stat -c '%s' "$OLD_AUDIT")" = 63990
test "$(sha256sum "$OLD_AUDIT" | awk '{print $1}')" = 41ddc3238d4cf8553b0981d4fe6971d899f6535b0bbdd10b941ba9d5effa424b
...
if audit['complete_coordinates'] != 17 or audit['comparison_rows'] != 119:
    raise ValueError('Unexpected current audit dimensions')
if old_audit['complete_coordinates'] != 16 or old_audit['comparison_rows'] != 112:
...
if len(artifacts) != 92 or len(old_artifacts) != 87:
...
if verified_bytes != 873147064 or len(verified_hashes) != 76:
...
if len(added_sources) != 5 or added_source_bytes != 48318119:
    raise ValueError(
