...
echo "=== FAILURES 7d ==="
sacct -X -n -P -S $(date -d '7 days ago' +%Y-%m-%d) --format=JobID,JobName,State,ExitCode,End 2>/dev/null | grep -E 'N22' | grep -E '\|(TIMEOUT|FAILED|NODE_FAIL|OUT_OF_MEMORY|CANCELLED)' || echo '  none'
...
echo "=== GATE ==="
grep -o '"released_code_numerical_status": *"[A-Z_]*"' "$ROOT/closure_20260810/aggregation/final_reproduction_gate.json" || echo '  gate missing'
exit 0
