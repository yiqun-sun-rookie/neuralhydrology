...
echo "=== N22 JOBS SINCE 09-03 ==="
sacct -X -n -P -S 2026-09-03 --format=JobID,JobName,State,ExitCode,Elapsed,End 2>/dev/null | grep -E 'N22' || echo '  none'
echo "=== WARMPAIR 219423 ==="
sacct -j 219423 -X -n -P --format=JobID,State,ExitCode,Elapsed,End 2>/dev/null || echo '  not found'
echo "=== WARMPAIR OUTPUTS ==="
...
echo "=== prepare_warmup_target_pair.py line 85-100 ==="
sed -n '85,100p' "$ROOT/src/29_nearing2022_da_ar/scripts/prepare_warmup_target_pair.py" 2>/dev/null || echo '  file missing'
echo "=== GATE ARTIFACTS ==="
