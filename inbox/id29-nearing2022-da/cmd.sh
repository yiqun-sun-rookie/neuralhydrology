#!/bin/bash
# ID29 seq=272: verify and recover the 21-coordinate isolated audit when terminal.
set -eo pipefail

ROOT=/data1/home/sunyiq/nearing2022_da
JOB_ID=202778
JOB_NAME=N22-part-audit10
TARGET=N22-EVAL-TS-DA-L08-TE000-S0
FINAL="$ROOT/results/29_nearing2022_da_ar/formal_closure/diagnostics/partial_numerical_audit_seq271_v1"
PREVIOUS="$ROOT/results/29_nearing2022_da_ar/formal_closure/diagnostics/partial_numerical_audit_seq249_v1"
...
if test "$STATE" != "COMPLETED" || test "$EXIT_CODE" != "0:0"; then
  ...
  for staging in "$(dirname "$FINAL")"/partial_numerical_audit_seq271_v1.preparing-*; do
...
if audit["complete_coordinates"] != 21:
    raise ValueError("Terminal audit does not contain exactly 21 complete coordinates")
if audit["comparison_rows"] != 147:
    raise ValueError("Terminal audit does not contain exactly 147 metric comparisons")
...
if role["partial_complete_count"] != 21 or role["partial_complete_count"] != audit["complete_coordinates"]:
...
if len(old_coordinates) != 20 or len(new_coordinates) != 21:
...
if receipt["mailbox_sequence"] != 271:
...
summary = {
    "schema": "nearing2022-partial-numerical-audit-seq271-terminal-verification-v1",
    "job_id": job_id,
    "slurm_node": node,
