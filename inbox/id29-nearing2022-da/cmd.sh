#!/bin/bash
# ID29 seq=251: independently rehash and closure-enumerator-cover all 107 seq=249 audit sources.
set -eo pipefail

ROOT=/data1/home/sunyiq/nearing2022_da
IDEA="$ROOT/src/29_nearing2022_da_ar"
AUDIT="$ROOT/results/29_nearing2022_da_ar/formal_closure/diagnostics/partial_numerical_audit_seq249_v1/audit_1.json"
OLD_AUDIT="$ROOT/results/29_nearing2022_da_ar/formal_closure/diagnostics/partial_numerical_audit_seq245_v1/audit_1.json"
ROLE="$ROOT/results/29_nearing2022_da_ar/formal_closure/diagnostics/partial_numerical_audit_seq249_v1/role_snapshot.json"
...
test "$(stat -c '%s' "$AUDIT")" = 78876
test "$(sha256sum "$AUDIT" | awk '{print $1}')" = 49fe6df436f663bea770ec08d23926d4acf44bdeec5791f06f3c2af6c80b2cb7
test "$(stat -c '%s' "$OLD_AUDIT")" = 75160
test "$(sha256sum "$OLD_AUDIT" | awk '{print $1}')" = 2c02a4c6a67f657775cc0938aa248c5737cea2d7d3c02a3082d8498ce2f3869f
test "$(stat -c '%s' "$ROLE")" = 2935
test "$(sha256sum "$ROLE" | awk '{print $1}')" = 4a6adf9933aafd3f27e4227063a112a43f8bc9d4b34f97497a82c5e593f37a5b
...
if audit['complete_coordinates'] != 20 or audit['comparison_rows'] != 140:
    raise ValueError('Unexpected current audit dimensions')
if old_audit['complete_coordinates'] != 19 or old_audit['comparison_rows'] != 133:
...
if role['slurm_job_id'] != '202754' or role['slurm_node'] != 'ngu102':
    raise ValueError('Role snapshot execution identity differs')
if role['partial_complete_count'] != 20 or role['closure_complete_count'] != 20:
    raise ValueError('Role snapshot count differs')
if role['target'] != 'N22-EVAL-TS-DA-L04-TE100-S0':
...
if len(artifacts) != 107 or len(old_artifacts) != 102:
...
if verified_bytes != 1018056276 or len(verified_hashes) != 87:
...
expected_added_coordinates = ['N22-EVAL-TS-DA-L04-TE100-S0']
...
if len(added_sources) != 5 or added_source_bytes != 48292940:
    raise ValueError(
        f'Unexpected added sources: count={len(added_sources)}, bytes={added_source_bytes}'
    )
print(json.dumps({
    'schema': 'nearing2022-partial-source-rehash-and-final-manifest-coverage-v6',
