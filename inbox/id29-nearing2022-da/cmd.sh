#!/bin/bash
# ID29 seq=250: verify and recover the 20-coordinate isolated audit when terminal.
set -eo pipefail

ROOT=/data1/home/sunyiq/nearing2022_da
JOB_ID=202754
JOB_NAME=N22-part-audit9
TARGET=N22-EVAL-TS-DA-L04-TE100-S0
FINAL="$ROOT/results/29_nearing2022_da_ar/formal_closure/diagnostics/partial_numerical_audit_seq249_v1"
PREVIOUS="$ROOT/results/29_nearing2022_da_ar/formal_closure/diagnostics/partial_numerical_audit_seq245_v1"
...
if test "$STATE" != "COMPLETED" || test "$EXIT_CODE" != "0:0"; then
  ...
  for staging in "$(dirname "$FINAL")"/partial_numerical_audit_seq249_v1.preparing-*; do
...
if audit["complete_coordinates"] != 20:
    raise ValueError("Terminal audit does not contain exactly 20 complete coordinates")
if audit["comparison_rows"] != 140:
    raise ValueError("Terminal audit does not contain exactly 140 metric comparisons")
...
if role["partial_complete_count"] != 20 or role["partial_complete_count"] != audit["complete_coordinates"]:
...
if len(old_coordinates) != 19 or len(new_coordinates) != 20:
...
if receipt["mailbox_sequence"] != 249:
...
summary = {
    "schema": "nearing2022-partial-numerical-audit-seq249-terminal-verification-v1",
    "job_id": job_id,
    "slurm_node": node,
