#!/usr/bin/env bash
set -eo pipefail
/data1/home/sunyiq/miniconda3/envs/nh_final/bin/python -B - <<'ZJ_PROBE_STATUS_PY'
import base64, hashlib, json, os, stat, subprocess
from pathlib import Path
from datetime import datetime, timezone, timedelta
root = Path('/data1/home/sunyiq/zhenjiang_filter_probe_20260911_001')
job_id = '225185'
expected_manifest = 'a86b2658acdf0854ceb18df9c6ae5b56068f244fc2ca96b344749320431feb07'
def safe(path):
...
report = {'schema_version': 'zhenjiang-probe-readonly-status-v3', 'job_id': job_id,
...
'evidence/probe_attempt_001/attempt.json','evidence/probe_attempt_001/failure.json','evidence/probe_attempt_001/completion.json',
'runs/probe_2022/completion.json','runs/probe_2022/probe_summary.json']
report['files'] = [file_record(name, include=True) for name in names]
for name in ['runs/probe_2022/probe_diagnostics.npz','evidence/probe_input_usage.sqlite3']:
    report['files'].append(file_record(name))
for name in ['logs/slurm-'+job_id+'.out','logs/slurm-'+job_id+'.err']:
    report['files'].append(file_record(name, tail_bytes=30000))
