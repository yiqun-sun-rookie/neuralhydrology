#!/bin/bash
# TUKF09-455 v2r8 status. Read-only, submits nothing, changes nothing.
set -o pipefail
...
ls "$ROOT/status" 2>&1
if [ -d "$ROOT/runtime_v2r8" ]; then echo "PRIVATE_RUNTIME_PRESENT $(du -sh "$ROOT/runtime_v2r8" | cut -f1)"; else echo PRIVATE_RUNTIME_ABSENT; fi
...
echo TUKF09_455_V2R8_STATUS_READ_ONLY
