#!/bin/bash
# TUKF09-455 v2r7: if the runtime inputs are published, submit exactly one preparation job.
set -o pipefail
...
JOB="$ROOT/bundle/kalmannet/hpc/tukf09_455_basin_revision_a800_exclusive_v2r7/probe_gpu.slurm"
...
if [ -d "$ROOT/offline_inputs_v2r7" ]; then
  echo "OFFLINE_INPUTS_PUBLISHED"
  du -sh "$ROOT/offline_inputs_v2r7"
  sha256sum "$ROOT/offline_inputs_v2r7/manifest.json"
  find "$ROOT/offline_inputs_v2r7" -type f | wc -l
else
  echo "OFFLINE_INPUTS_NOT_READY"
  du -sh "$ROOT"/offline_inputs_v2r7.pending.* 2>&1
  tail -c 500 "$ROOT/logs/offline-inputs-download.log" 2>&1
  echo "TUKF09_455_V2R7_WAITING_FOR_OFFLINE_INPUTS_NO_JOB_SUBMITTED"
  exit 0
fi

echo "=== SUBMISSION PRECONDITIONS ==="
test -f "$JOB" && echo "JOB_SCRIPT_PRESENT" || { echo "JOB_SCRIPT_MISSING"; exit 1; }
grep -n "SBATCH --exclusive\|SBATCH --exclude" "$JOB" 2>&1
test ! -e "$ROOT/runtime_v2r7" && echo "PRIVATE_RUNTIME_ABSENT_AS_EXPECTED" || echo "PRIVATE_RUNTIME_ALREADY_PRESENT"
...
echo "TUKF09_455_V2R7_PREPARATION_SUBMITTED_ONCE_NO_TRAINING_NO_FORMAL_EVALUATION"
