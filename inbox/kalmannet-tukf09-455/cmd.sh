#!/bin/bash
# TUKF09-455 v2r6: read-only preparation status plus queue position. Submits nothing.
set -o pipefail
...
echo "--- queue reason and estimated start ---"
squeue -j "$JID" -h -o "%T | %R | elapsed=%M | start=%S" 2>&1
echo "--- partition ---"
sinfo -p hgpu8 -o "%.10P %.6a %.6D %.8t %.24N %.20C" 2>&1
echo "=== MARKERS ==="
for f in PREPARATION_FAILED.json initial_bundle_verification.json staged_training_sources.json preparation_probe.json hpc_technical_admission.json; do
...
if [ -d "$R" ]; then echo "FILTER_UNITS=$(ls "$R/filter" 2>/dev/null | wc -l)"; else echo "RESULT_ROOT_ABSENT"; fi
echo "=== LOG TAIL ==="
tail -c 2000 "$ROOT/logs/prepare-$JID.out" 2>&1
tail -c 1000 "$ROOT/logs/prepare-$JID.err" 2>&1
