#!/bin/bash
# TUKF09-455 v2r10: submit exactly one preparation job once the runtime inputs are
...
JOB="$ROOT/bundle/kalmannet/hpc/tukf09_455_basin_revision_a800_exclusive_v2r10/probe_gpu.slurm"
...
if [ -d "$ROOT/offline_inputs_v2r10" ]; then
  echo OFFLINE_INPUTS_PUBLISHED
  du -sh "$ROOT/offline_inputs_v2r10"
  sha256sum "$ROOT/offline_inputs_v2r10/manifest.json"
  echo "OFFLINE_FILES=$(find "$ROOT/offline_inputs_v2r10" -type f | wc -l)"
else
  echo OFFLINE_INPUTS_NOT_READY
  du -sh "$ROOT"/offline_inputs_v2r10.pending.* 2>&1
  tail -c 500 "$ROOT/logs/offline-inputs-download.log" 2>&1
  echo TUKF09_455_V2R10_WAITING_FOR_OFFLINE_INPUTS_NO_JOB_SUBMITTED
  exit 0
fi
...
test ! -e "$ROOT/runtime_v2r10" && echo PRIVATE_RUNTIME_ABSENT_AS_EXPECTED || echo PRIVATE_RUNTIME_ALREADY_PRESENT
...
echo TUKF09_455_V2R10_PREPARATION_SUBMITTED_ONCE_NO_TRAINING_NO_FORMAL_EVALUATION
