#!/bin/bash
# TUKF09-455: deploy the v2r12 bundle into a fresh remote root and start the offline
...
CAPSULE=/data1/home/sunyiq/kalmannet_tukf09_455_basin_zero_validation_target_variance_revision_v1_training_source_capsule_v6_20260908
PAY=/data1/home/sunyiq/hpc_mailbox/payload/kalmannet-tukf09-455/v2r12
ARCHIVE=tukf09_455_basin_zero_validation_target_variance_revision_v1_hpc_execution_a800_exclusive_v2r12_formal_training.tar.gz
echo "TIME=$(date -Is)"
...
if [ "$ACT_SHA" != "8bcf38a70e16c48e1afcfc5d878195d3ef8213cafd33aae9fdd999d33f4a9db6" ] || [ "$ACT_SIZE" != "9924050" ]; then
...
python -X utf8 "$PAY/build_tukf09_455_a800_exclusive_hpc_bundle_v2r12.py" \
  --archive "$PAY/$ARCHIVE" --extract-to "$ROOT/bundle"
python -X utf8 "$ROOT/bundle/kalmannet/scripts/build_tukf09_455_a800_exclusive_hpc_bundle_v2r12.py" \
  --verify-extracted "$ROOT/bundle"

echo "=== BINDINGS THIS REVISION MUST HAVE ==="
P="$ROOT/bundle/kalmannet"
S="$P/hpc/tukf09_455_basin_revision_a800_exclusive_v2r12/submit_training_gpu.slurm"
echo "CAPSULE_BINDING=$(grep -c "training_source_capsule_v6_20260908" "$P/hpc/tukf09_455_basin_revision_a800_exclusive_v2r12/probe_gpu.slurm")"
...
echo "MIGRATION_PIN=$(python -X utf8 -c "import json;print(json.load(open('$P/configs/tukf09_455_basin_zero_validation_target_variance_hpc_execution_a800_exclusive_v2r12.json'))['scientific_identity']['filter_migration_final_manifest']['sha256'])")"

echo "=== START THE RUNTIME INPUT DOWNLOAD, DETACHED ==="
D="$P/hpc/tukf09_455_basin_revision_a800_exclusive_v2r12/download_runtime_inputs_login.sh"
