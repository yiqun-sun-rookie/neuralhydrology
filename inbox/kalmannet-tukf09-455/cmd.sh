#!/bin/bash
# TUKF09-455 v2r6 progress check. Read-only. Submits nothing, changes nothing.
set -o pipefail
...
squeue -j "$JID" -h -o "QUEUE %T | %R | elapsed=%M | start=%S | priority=%Q" 2>&1
echo "=== PARTITION ==="
sinfo -p hgpu8 -o "%.10P %.6a %.6D %.8t %.24N %.20C" 2>&1
echo "=== PENDING EXCLUSIVE COMPETITION ON hgpu8 ==="
squeue -p hgpu8 -h -o "%.10i %.9u %.9T %.14R %.9M %.6D" 2>&1
echo "=== STATUS MARKERS ==="
...
if [ -d "$ROOT/runtime_v2r6" ]; then echo "PRESENT $(du -sh "$ROOT/runtime_v2r6" | cut -f1)"; else echo "ABSENT"; fi
echo "=== RESULT ROOT IN BUNDLE ==="
...
tail -c 700 "$ROOT/logs/prepare-$JID.err" 2>&1
