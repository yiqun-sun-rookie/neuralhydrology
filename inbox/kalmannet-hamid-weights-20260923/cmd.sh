...
sacct -X -j 227726,227732 -n -P --format=JobIDRaw,JobName,State,ExitCode,Elapsed,NodeList || true
printf '\nSIXTH_COMPLETED_CELL_INDEPENDENT_AUDIT\n'
...
out=root/'formal_attempt_003/runs/objective-current-0p05-highflow-1-seed-42'
...
cell=protocol['cells'][6]
assert cell['exp_id']==out.name and cell['lambda_upd']==0.05 and cell['alpha_hf']==1 and cell['seed']==42
...
assert start['cell']==cell and start['job_id']=='227726' and start['array_identity']=='227626_6'
...
assert len(epochs)==complete['epochs_completed']==16 and [e['epoch'] for e in epochs]==list(range(16))
...
assert len(replays)==complete['recovery_total']==3 and [e['recovery_total'] for e in replays]==[1,2,3]
assert all(e['epoch'] in range(16) for e in replays)
best=min(epochs,key=lambda e:e['common_validation_mse'])
assert best['epoch']==complete['best_epoch']==5 and best['common_validation_mse']==complete['best_validation_mse']
assert complete['best_checkpoint']=='checkpoint_epoch_005.pt'
