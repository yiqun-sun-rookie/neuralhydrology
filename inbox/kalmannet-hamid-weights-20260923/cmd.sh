...
printf '\nSEVENTH_CELL_READ_ONLY_CURRENT_AUDIT\n'
...
assert len(epochs)>=40 and [e['epoch'] for e in epochs]==list(range(len(epochs)))
...
assert [e['recovery_total'] for e in replays]==list(range(1,len(replays)+1))
...
PY
printf '\nREAD_ONLY_CURRENT_AUDIT_COMPLETE\n'
