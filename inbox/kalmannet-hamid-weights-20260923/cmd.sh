...
sacct -X -j 227732,227745 -n -P --format=JobIDRaw,JobName,State,ExitCode,Elapsed,NodeList || true
printf '\nSEVENTH_COMPLETED_CELL_INDEPENDENT_AUDIT\n'
...
cell=protocol['cells'][7]
assert cell['exp_id']==out.name and cell['lambda_upd']==0.05 and cell['alpha_hf']==1 and cell['seed']==43
...
assert len(epochs)==complete['epochs_completed']==45 and [e['epoch'] for e in epochs]==list(range(45))
...
assert all(e['epoch'] in range(45) for e in replays)
best=min(epochs,key=lambda e:e['common_validation_mse'])
assert best['epoch']==complete['best_epoch']==34 and best['common_validation_mse']==complete['best_validation_mse']
assert complete['best_checkpoint']=='checkpoint_epoch_034.pt'
