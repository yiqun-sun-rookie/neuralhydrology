...
attempt="$root/diagnostic_20260924_v1"
job_id=$(cat "$root/diagnostic_submission_claim_20260924_v1/job_id.txt")
[[ "$job_id" =~ ^[0-9]+$ ]]
...
for path in "$attempt"/preflight_cpu/result.json "$attempt"/preflight_gpu/result.json "$attempt"/HALT.json "$attempt"/runs/*/full_batch_observer_parity.json "$attempt"/runs/*/diagnostic_training_observation.json "$attempt"/runs/*/failure.json; do
 if [ -f "$path" ]; then printf '\nFILE=%s\n' "$path"; tail -c 6000 "$path"; fi
done
