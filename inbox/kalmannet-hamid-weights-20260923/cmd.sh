...
sacct -X -j 227671,227672 -n -P --format=JobIDRaw,JobName,State,ExitCode,Elapsed,NodeList || true
printf '\nSECOND_COMPLETED_CELL_INDEPENDENT_AUDIT\n'
...
out=root/'formal_attempt_003/runs/objective-current-0p05-highflow-2-seed-44'
...
assert start['cell']==cell and start['job_id']=='227671' and start['array_identity']=='227626_2'
...
assert len(epoch)==complete['epochs_completed']==18 and [e['epoch'] for e in epoch]==list(range(18))
...
assert len(replays)==complete['recovery_total']==1 and replays[0]['epoch']==4
best=min(epoch,key=lambda e:e['common_validation_mse'])
assert best['epoch']==complete['best_epoch']==17 and best['common_validation_mse']==complete['best_validation_mse']
assert complete['best_checkpoint']=='checkpoint_epoch_017.pt'
...
assert len(train)>=18
assert all(train[(e['epoch'],1 if e['epoch']==4 else 0)]==expected_train for e in epoch)
print(json.dumps({'state':'SECOND_CELL_TRAINING_INDEPENDENTLY_AUDITED','array_identity':start['array_identity'],'job_id':start['job_id'],'epochs_completed':len(epoch),'first_optimizer_update_verified':True,'recovery_total':len(replays),'best_epoch_zero_based':best['epoch'],'best_validation_mse':best['common_validation_mse'],'checkpoint_sha256':sha(out/complete['best_checkpoint']),'validation_samples_each_epoch':expected_val,'full_training_samples_each_completed_epoch':expected_train,'test_data_read':False,'scientific_comparison_complete':False}))
PY
