...
sacct -X -j 227749,227626 -n -P --format=JobIDRaw,JobName,State,ExitCode,Elapsed,NodeList || true
printf '\nTENTH_COMPLETED_CELL_INDEPENDENT_AUDIT\n'
...
out=root/'formal_attempt_003/runs/objective-current-0-highflow-1-seed-43'
...
cell=protocol['cells'][10]
assert cell['exp_id']==out.name and cell['lambda_upd']==0 and cell['alpha_hf']==1 and cell['seed']==43
...
assert start['cell']==cell and start['job_id']=='227749' and start['array_identity']=='227626_10'
...
assert len(replays)==complete['recovery_total']==1 and [e['recovery_total'] for e in replays]==[1]
assert all(e['epoch'] in range(16) for e in replays)
best=min(epochs,key=lambda e:e['common_validation_mse'])
assert best['epoch']==complete['best_epoch']==5 and best['common_validation_mse']==complete['best_validation_mse']==1.0443245598029039e-05
