...
ROOT="$TARGET/repo"
JOB_ID=$(tr -d '[:space:]' < "$TARGET/deployment/preselection_gates_job_id_v03.txt")
JOB_NUM=$(printf '%s' "$JOB_ID" | cut -d';' -f1)

echo "=== PRESELECTION SECOND RETRY STATUS ==="
date -Is
hostname
echo "job_id=$JOB_NUM"
...
reports = sorted(report_root.glob(f"id30_*_s100_slurm{job_id}/probe_report.json"))
if not reports:
...
for path in reports:
    try:
        ...
        print(json.dumps({"path": str(path.relative_to(root)), "status": "CORRUPT", "error": str(error)}))
        continue
    summary = {
