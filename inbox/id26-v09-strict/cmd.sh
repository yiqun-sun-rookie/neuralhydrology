#!/bin/bash
# id26-v09-strict seq=17 : SHORT status query, no waiting loop.
# seq=16 was killed by the mailbox 2 h worker timeout before it could push a result.
export LC_ALL=C
...
echo "=== JOB ==="
squeue -j "$JID" -o "%.10i %.10j %.8T %.12M %R" 2>&1 | head -3
sacct -j "$JID" -X --format=JobID%10,State%12,ExitCode%8,Elapsed%12 2>&1 | head -4

echo "=== RUN DIRECTORIES ==="
find "$CODE/results/26_historical_band_experts/formal_v09" -maxdepth 2 -name 'seed_*' 2>/dev/null | sort

echo "=== SUITE LOG (tail 25) ==="
tail -25 "$ROOT/logs/suite_${JID}.out" 2>&1
echo "=== SUITE ERR (tail 15) ==="
tail -15 "$ROOT/logs/suite_${JID}.err" 2>&1
...
for family in ("classic", "capacity", "continuous"):
    ...
    if not m.is_file():
        print(f"{family:11s}: not finished")
        continue
    ...
    print(f"{family:11s}: {d['status']} params={d['trainable_parameters']} steps={d['optimizer_steps_total']} "
          ...
          f"loss ep1={d['epoch_trace'][0]['mean_training_loss']:.5f} -> ep30={d['epoch_trace'][-1]['mean_training_loss']:.5f}")
