#!/bin/bash
# id26-v09-strict seq=60 : query training audit attempt 02 once; do not submit anything.
set -euo pipefail
export LC_ALL=C
...
JOBID_FILE=$AUDIT_PARENT/training_audit_attempt_02_jobid.txt
...
echo "--- stdout tail ---"
if [ -f "$OUT" ]; then tail -n 120 "$OUT"; else echo "MISSING $OUT"; fi
echo "--- stderr tail ---"
if [ -f "$ERR" ]; then tail -n 120 "$ERR"; else echo "MISSING $ERR"; fi
