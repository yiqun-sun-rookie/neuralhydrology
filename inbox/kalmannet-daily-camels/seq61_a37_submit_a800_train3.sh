...
RESULT60="${MAILBOX_ROOT}/outbox/kalmannet-daily-camels/result_60.txt"
RESULT60_SHA256="487d68008fa85d2e52bca965cfbc0a781cccc30aef87f2163bafcdb9ed127abf"
PROBE_JOB_ID="215366"
EXPERIMENT_ID="DAILY_CAMELS_KNET_FIXED_FULL_TRAINING_COVERAGE_EPOCH10_TO40_RESUME_V1_20260826_A37"
FAILED_EXECUTION_ATTEMPT_ID="DAILY_CAMELS_KNET_FIXED_FULL_TRAINING_COVERAGE_A37_A800_TRAIN2_SEQ60"
FAILED_RUN_BASE="/data1/home/sunyiq/kalmannet_daily_camels_knet_a37_a800_train2_20260827"
FAILED_RUN_DIRECTORY="${FAILED_RUN_BASE}/runs/${EXPERIMENT_ID}"
FAILED_JOB_ID_FILE="${FAILED_RUN_BASE}/status/seq60_a800_training_job_id.txt"
FAILED_IDENTITY_FILE="${FAILED_RUN_BASE}/status/seq60_a800_train_submission_identity.txt"
...
PAYLOAD_DIRECTORY="${MAILBOX_ROOT}/payload/kalmannet-daily-camels/a37_a800_train3_seq61_20260827"
...
WRAPPER_SOURCE="${MAILBOX_ROOT}/inbox/kalmannet-daily-camels/seq61_a37_a800_train3_wrapper.slurm"
WRAPPER="${RUN_BASE}/seq61_a37_a800_train3_wrapper.slurm"
WRAPPER_SHA256="4b617343afeea05c3b94f7c415874f6ce2ff2f3c5e6f7ded72de3cdbd289f88e"
WRAPPER_SIZE="2490"
OFFLINE_REPORT="${STATUS_DIRECTORY}/seq61_offline_train_bundle_verification.json"
IDENTITY_FILE="${STATUS_DIRECTORY}/seq61_a800_train_submission_identity.txt"
JOB_ID_FILE="${STATUS_DIRECTORY}/seq61_a800_training_job_id.txt"
SUBMITTED_TIME_FILE="${STATUS_DIRECTORY}/seq61_a800_training_submitted_time_utc.txt"
SQUEUE_BEFORE="${STATUS_DIRECTORY}/seq61_pre_submission_squeue.txt"
SQUEUE_AFTER="${STATUS_DIRECTORY}/seq61_post_submission_squeue.txt"
SACCT_AFTER="${STATUS_DIRECTORY}/seq61_post_submission_sacct.txt"
NODE_REPORT="${STATUS_DIRECTORY}/seq61_pre_submission_hgpu8_candidate_nodes.txt"
...
require_regular_identity "$RESULT60" "$RESULT60_SHA256" "11310" "sequence 60 failed-submission evidence"
grep -Fq 'sbatch: error: invalid number of nodes (-N 2-1)' "$RESULT60" || {
  echo "sequence 60 does not prove the recoverable Slurm node-list failure" >&2
  exit 54
}
grep -Fq '### exit_code=1' "$RESULT60" || {
  echo "sequence 60 failed-submission exit code differs" >&2
  exit 55
}
if grep -Fq 'SEQ60_A37_A800_TRAIN_SUBMITTED' "$RESULT60"; then
  echo "sequence 60 unexpectedly contains a successful submission marker" >&2
  exit 66
fi
for prior_evidence in "$FAILED_IDENTITY_FILE" "$FAILED_SUBMISSION_LOCK_OWNER"; do
  [[ -f "$prior_evidence" && ! -L "$prior_evidence" ]] || {
    echo "sequence 60 prior-attempt identity evidence is absent or symbolic: $prior_evidence" >&2
    exit 68
  }
  grep -Fxq "experiment_id=${EXPERIMENT_ID}" "$prior_evidence" || {
    echo "sequence 60 prior-attempt experiment identity differs" >&2
    exit 69
  }
  grep -Fxq "execution_attempt_id=${FAILED_EXECUTION_ATTEMPT_ID}" "$prior_evidence" || {
    echo "sequence 60 prior-attempt execution identity differs" >&2
    exit 70
  }
done
grep -Fxq 'wrapper_sha256=eacfee41743f658b4bf9f784fed4efbaf9e082bc698d71f02841e6b4b043a78b' "$FAILED_IDENTITY_FILE" || {
  echo "sequence 60 prior-attempt wrapper identity differs" >&2
  exit 71
}
if [[ -e "$FAILED_JOB_ID_FILE" || -L "$FAILED_JOB_ID_FILE" || -e "$FAILED_RUN_DIRECTORY" || -L "$FAILED_RUN_DIRECTORY" ]]; then
  echo "sequence 60 created a job identifier or run directory; refusing retry" >&2
  exit 67
fi
...
IDENTITY_TEMP="$(mktemp "${STATUS_DIRECTORY}/seq61_identity.XXXXXX")"
{
  printf 'experiment_id=%s\n' "$EXPERIMENT_ID"
  printf 'execution_attempt_id=%s\n' "$EXECUTION_ATTEMPT_ID"
...
JOB_ID_TEMP="$(mktemp "${STATUS_DIRECTORY}/seq61_job_id.XXXXXX")"
printf '%s\n' "$TRAINING_JOB_ID" > "$JOB_ID_TEMP"
ln -- "$JOB_ID_TEMP" "$JOB_ID_FILE"
rm -- "$JOB_ID_TEMP"
TIME_TEMP="$(mktemp "${STATUS_DIRECTORY}/seq61_time.XXXXXX")"
...
printf 'SEQ61_A37_A800_TRAIN_SUBMITTED experiment_id=%s execution_attempt_id=%s training_job_id=%s run_base=%s archive_sha256=%s wrapper_sha256=%s source_checkpoint_sha256=%s portability_receipt_sha256=%s target_partition=hgpu8 excluded_node=ngu201\n' \
