...
RESULT78="${MAILBOX_ROOT}/outbox/kalmannet-daily-camels/result_78.txt"
...
COLLECTOR78="${MAILBOX_ROOT}/inbox/kalmannet-daily-camels/seq78_a38_a800_job215801_terminal_recollect.sh"
...
RESULT77_SIZE="1495"
RESULT78_SHA256="c2d9b1781f08ad846c1ac98d3b5c6fbfc9f4cedc04af042f6a1d6a4cccbcdb5f"
RESULT78_SIZE="7397905"
RESULT80_SHA256="2446ca2fab69bbdbc98c47e083f4d0a8ad6a86d1a01b684d1f0d7d95582c1589"
RESULT80_SIZE="7398611"
RESULT81_SHA256="7c6c58590f090e722690ccdb455179e1627111bae47d472f2e340661ec760265"
RESULT81_SIZE="2388"
COLLECTOR78_SHA256="db6de5b13eb6c49c9ceda3be560ca753e6a077ae11b525055d9f6f32851d5602"
COLLECTOR78_SIZE="73233"
COLLECTOR79_SHA256="23637bdf983fc93766f16a040146ee9c7cb1bda383ef25b978f41723b56fde7a"
COLLECTOR79_SIZE="75864"
JOB_ID="215801"
...
CGROUP_RESOURCE_LOG_SHA256="19eb277e7533c457f8278e004640cc00b6a6228979e5bb6fd7db620d49e843ea"
CGROUP_RESOURCE_LOG_SIZE="129"
...
FAILED_ARCHIVE78="${MAILBOX_ROOT}/outbox/kalmannet-daily-camels/DAILY_CAMELS_KNET_A38_JOB215801_TERMINAL_EVIDENCE_SEQ78.tar.gz"
FAILED_ARCHIVE79="${MAILBOX_ROOT}/outbox/kalmannet-daily-camels/DAILY_CAMELS_KNET_A38_JOB215801_TERMINAL_EVIDENCE_SEQ79.tar.gz"
ARCHIVE="${MAILBOX_ROOT}/outbox/kalmannet-daily-camels/DAILY_CAMELS_KNET_A38_JOB215801_TERMINAL_EVIDENCE_SEQ82.tar.gz"
...
require_regular_identity "$RESULT78" "$RESULT78_SHA256" "$RESULT78_SIZE" "sequence 78 failed collection receipt"
require_regular_identity "$RESULT80" "$RESULT80_SHA256" "$RESULT80_SIZE" "sequence 80 failed collection receipt"
require_regular_identity "$RESULT81" "$RESULT81_SHA256" "$RESULT81_SIZE" "sequence 81 global-gate diagnostic receipt"
require_regular_identity "$COLLECTOR78" "$COLLECTOR78_SHA256" "$COLLECTOR78_SIZE" "sequence 78 failed collector"
require_regular_identity "$COLLECTOR79" "$COLLECTOR79_SHA256" "$COLLECTOR79_SIZE" "sequence 79 failed collector"
...
grep -Fxq 'SEQ78_A38_GPU_RESOURCE_SUMMARY_END' "$RESULT78" || {
  echo "sequence 78 GPU resource gate did not complete" >&2
  exit 57
}
grep -Fxq 'A38 numeric cgroup memory peak is absent' "$RESULT78" || {
  echo "sequence 78 recoverable cgroup collection failure differs" >&2
  exit 57
}
grep -Fxq '### exit_code=66' "$RESULT78" || {
  echo "sequence 78 mailbox exit code differs" >&2
  exit 57
}
if grep -Fq 'SEQ78_A38_INDEPENDENT_VALIDATION ' "$RESULT78" || grep -Fq 'SEQ78_A38_TERMINAL_COLLECTED ' "$RESULT78"; then
  echo "sequence 78 unexpectedly reached independent validation or archive publication" >&2
  exit 57
fi
grep -Fxq '### channel=kalmannet-daily-camels seq=80' "$RESULT80" || { echo "sequence 80 receipt header differs" >&2; exit 57; }
grep -Fxq 'independently recomputed global training gate differs' "$RESULT80" || { echo "sequence 80 collector failure differs" >&2; exit 57; }
grep -Fxq '### exit_code=1' "$RESULT80" || { echo "sequence 80 mailbox exit code differs" >&2; exit 57; }
grep -Fxq '### channel=kalmannet-daily-camels seq=81' "$RESULT81" || { echo "sequence 81 receipt header differs" >&2; exit 57; }
grep -Fq '"failed_condition_names":["declared_improvement_exact"]' "$RESULT81" || { echo "sequence 81 failed-condition diagnosis differs" >&2; exit 57; }
grep -Fq '"overall_improvement_difference_declared_minus_recomputed":2.2920620956767834e-10' "$RESULT81" || { echo "sequence 81 cross-device difference diagnosis differs" >&2; exit 57; }
grep -Fxq 'SEQ81_A38_GLOBAL_GATE_DIAGNOSTIC_COMPLETE' "$RESULT81" || { echo "sequence 81 diagnostic completion marker differs" >&2; exit 57; }
grep -Fxq '### exit_code=0' "$RESULT81" || { echo "sequence 81 mailbox exit code differs" >&2; exit 57; }
...
[[ ! -e "$FAILED_ARCHIVE78" && ! -L "$FAILED_ARCHIVE78" ]] || {
  echo "failed sequence 78 archive unexpectedly exists" >&2
  exit 57
}
[[ ! -e "$FAILED_ARCHIVE79" && ! -L "$FAILED_ARCHIVE79" ]] || {
  echo "failed sequence 79 archive unexpectedly exists" >&2
  exit 57
}
[[ ! -e "$ARCHIVE" && ! -L "$ARCHIVE" ]] || {
  echo "sequence 82 terminal evidence archive already exists" >&2
  exit 57
}
...
STEP_ACCOUNTING="$(sacct -n -j "$JOB_ID" --units=K --parsable2 --format=JobIDRaw,State,ExitCode,Start,End,ElapsedRaw,MaxRSS,MaxVMSize)"
...
IFS='|' read -r _ TRAINING_STEP_STATE TRAINING_STEP_EXIT TRAINING_STEP_START TRAINING_STEP_END TRAINING_STEP_ELAPSED_SECONDS TRAINING_STEP_MAX_RSS TRAINING_STEP_MAX_VM_SIZE _ <<<"$TRAINING_STEP_LINE"
...
[[ "$TRAINING_STEP_MAX_RSS" =~ ^([0-9]+)K$ ]] || { echo "A38 training-step Slurm MaxRSS is absent or nonnumeric" >&2; exit 59; }
TRAINING_STEP_MAX_RSS_K="${BASH_REMATCH[1]}"
[[ "$TRAINING_STEP_MAX_VM_SIZE" =~ ^([0-9]+)K$ ]] || { echo "A38 training-step Slurm MaxVMSize is absent or nonnumeric" >&2; exit 59; }
TRAINING_STEP_MAX_VM_SIZE_K="${BASH_REMATCH[1]}"
[[ "$TRAINING_STEP_MAX_RSS_K" -gt 0 && "$TRAINING_STEP_MAX_VM_SIZE_K" -gt 0 ]] || { echo "A38 training-step Slurm memory evidence is nonpositive" >&2; exit 59; }
TRAINING_STEP_MAX_RSS_BYTES="$((TRAINING_STEP_MAX_RSS_K * 1024))"
TRAINING_STEP_MAX_VM_SIZE_BYTES="$((TRAINING_STEP_MAX_VM_SIZE_K * 1024))"
set +e
SQUEUE_SNAPSHOT="$(squeue -h -j "$JOB_ID" -o '%A|%j|%P|%T|%R|%b|%Z|%o' 2>&1)"
SQUEUE_EXIT_CODE="$?"
set -e
printf '%s\n' 'SEQ82_A38_SQUEUE_BEGIN' "$SQUEUE_SNAPSHOT" 'SEQ82_A38_SQUEUE_END'
...
for path in "$LOG_OUT" "$LOG_ERR" "$PREFLIGHT_REPORT" "$CGROUP_RESOURCE_LOG" \
  ...
  if [[ -f "$path" && ! -L "$path" ]] && ! reserved_named_path "$path"; then
    printf 'SEQ82_A38_CONTENT_BEGIN path=%s size=%s sha256=%s\n' "$path" "$(stat -c '%s' "$path")" "$(sha256_file "$path")"
    sed -n '1,30000p' "$path"
    printf 'SEQ82_A38_CONTENT_END path=%s\n' "$path"
  else
    printf 'SEQ82_A38_CONTENT_ABSENT_OR_UNSAFE path=%s\n' "$path"
  fi
done
...
printf '%s\n' 'SEQ82_A38_GPU_RESOURCE_SUMMARY_BEGIN'
...
printf '%s\n' 'SEQ82_A38_GPU_RESOURCE_SUMMARY_END'

require_regular_identity "$CGROUP_RESOURCE_LOG" "$CGROUP_RESOURCE_LOG_SHA256" "$CGROUP_RESOURCE_LOG_SIZE" "A38 cgroup resource log"
grep -Fxq "slurm_job_id=${JOB_ID}" "$CGROUP_RESOURCE_LOG" || { echo "A38 cgroup job identity differs" >&2; exit 66; }
grep -Fxq 'process_cgroup_relative=unavailable' "$CGROUP_RESOURCE_LOG" || { echo "A38 cgroup process path status differs" >&2; exit 66; }
grep -Fxq 'cgroup_v2_memory_status=unavailable' "$CGROUP_RESOURCE_LOG" || { echo "A38 cgroup v2 memory status differs" >&2; exit 66; }
if grep -Eq '^(current_memory_peak|parent_memory_peak)=' "$CGROUP_RESOURCE_LOG"; then
  echo "A38 cgroup log mixes unavailable and numeric peak states" >&2
  exit 66
fi
printf 'SEQ82_A38_HOST_MEMORY_RESOURCE_SUMMARY slurm_job_id=%s cgroup_memory_peak_status=unavailable slurm_training_step_max_rss_k=%s slurm_training_step_max_rss_bytes=%s slurm_training_step_max_vm_size_k=%s slurm_training_step_max_vm_size_bytes=%s\n' \
  "$JOB_ID" "$TRAINING_STEP_MAX_RSS_K" "$TRAINING_STEP_MAX_RSS_BYTES" "$TRAINING_STEP_MAX_VM_SIZE_K" "$TRAINING_STEP_MAX_VM_SIZE_BYTES"

python - "$RUN_DIRECTORY" "$PREFLIGHT_REPORT" "$SOURCE_RESULT_SUMMARY" "$EXPERIMENT_ID" "$SOURCE_CHECKPOINT_SHA256" "$SOURCE_EPOCH_HISTORY_SHA256" "$SOURCE_EPOCH40_PARAMETER_SHA256" "$SOURCE_CONFIGURATION_SHA256" "$SOURCE_IDENTITY_SHA256" "$SOURCE_RESULT_SUMMARY_SHA256" "$SOURCE_EVIDENCE_ARCHIVE_SHA256" "$ACTIVE_CONFIG_SHA256" "$JOB_ID" "$NORMALIZED_A38_REPLAY_SHA256" "$RESULT80_SHA256" "$RESULT81_SHA256" <<'PY'
import copy
import hashlib
import json
import math
...
result80_sha = sys.argv[15]
result81_sha = sys.argv[16]
...
# Epochs 0-40 are the immutable RTX 3090 source history.  The resumed A800 run
# independently replayed epoch 0 on A800 and the frozen scientific gate used
# that target-device value.  Preserve both exact endpoints instead of treating
# their already-registered cross-device floating-point difference as a result
# inconsistency.
migration_identity = continuation_identity.get("gate_revision", {}).get(
    "epoch_zero_reproducibility_migration", {}
)
migration_training = continuation.get("gate_revision", {}).get(
    "epoch_zero_reproducibility_migration", {}
)
epoch_zero_reproducibility = training.get("epoch_zero_reproducibility", {})
source_epoch_zero_objective = history_objectives[0]
target_epoch_zero_objective = finite_float(
    training.get("epoch_zero_checkpoint_objective"),
    "A800 epoch-zero checkpoint objective",
    nonnegative=True,
)
cross_device_epoch_zero_difference = (
    target_epoch_zero_objective - source_epoch_zero_objective
)
if (
    migration_identity != migration_training
    or finite_float(
        migration_identity.get("source_checkpoint_objective_728"),
        "registered RTX 3090 epoch-zero objective",
        nonnegative=True,
    )
    != source_epoch_zero_objective
    or finite_float(
        migration_identity.get("target_checkpoint_objective_728"),
        "registered A800 epoch-zero objective",
        nonnegative=True,
    )
    != target_epoch_zero_objective
    or migration_identity.get("source_device_name") != "NVIDIA GeForce RTX 3090"
    or migration_identity.get("target_device_name") != "NVIDIA A800-SXM4-80GB"
    or migration_identity.get("source_portability_receipt_sha256")
    != "747c304f1ec12d551d4b8b6f6a525fe28c9d757313c9fd009a741f8a3dcfacae"
    or migration_identity.get("parameter_sha256")
    != training.get("epoch_zero_parameter_sha256")
    or epoch_zero_reproducibility.get("parameter_sha256")
    != training.get("epoch_zero_parameter_sha256")
    or migration_identity.get("source_prediction_sha256")
    != history[0].get("fixed_window_prediction_sha256")
    or migration_identity.get("target_prediction_sha256")
    != epoch_zero_reproducibility.get("prediction_sha256")
    or finite_float(
        epoch_zero_reproducibility.get("checkpoint_objective_728"),
        "A800 epoch-zero replay objective",
        nonnegative=True,
    )
    != target_epoch_zero_objective
    or epoch_zero_reproducibility.get("required") is not True
    or epoch_zero_reproducibility.get("status") != "PASS"
    or cross_device_epoch_zero_difference != 2.2920620956767834e-10
):
    raise SystemExit("registered RTX 3090 to A800 epoch-zero migration differs")

overall_improvement = target_epoch_zero_objective - global_best_objective
if (
...
audit = {
    ...
    "global_best_checkpoint_objective": global_best_objective,
    "global_checkpoint_objective_improvement": overall_improvement,
    "source_rtx3090_epoch_zero_checkpoint_objective": source_epoch_zero_objective,
    "target_a800_epoch_zero_checkpoint_objective": target_epoch_zero_objective,
    "registered_cross_device_epoch_zero_difference": cross_device_epoch_zero_difference,
    ...
    "terminal_collection_repair": {
        "reason": "use_registered_A800_epoch_zero_for_global_gate",
        "result_80_sha256": result80_sha,
        "result_81_sha256": result81_sha,
        "strict_source_and_target_endpoint_binding": True,
    },
}
print("SEQ82_A38_INDEPENDENT_VALIDATION " + json.dumps(audit, sort_keys=True, separators=(",", ":")))
PY
