...
printf 'SEQ100_A39_TERMINAL_DIAGNOSTIC experiment_id=%s execution_id=%s job_id=%s\n' \
  "${EXPERIMENT_ID}" "${EXECUTION_ID}" "${JOB_ID}"
set +e
sacct -n -X -j "${JOB_ID}" \
  --format=JobIDRaw,JobName,User,Partition,State,ExitCode,Elapsed,Start,End,NodeList,MaxRSS,MaxVMSize -P
SACCT_EXIT=$?
set -e
...
for directory in \
  "${RUN_ROOT}" \
  "${RUN_ROOT}/source_A39_formal_evaluation_seq98" \
  "${RUN_ROOT}/logs" \
  "${STATUS_ROOT}" \
  "${STATUS_ROOT}/locks" \
  "${RUN_ROOT}/inputs" \
...
show_identity_file() {
  local path="$1" label="$2"
  if [[ -f "${path}" && ! -L "${path}" ]]; then
    printf 'IDENTITY_FILE label=%s path=%s size=%s sha256=%s\n' \
      "${label}" "${path}" "$(stat -c '%s' "${path}")" "$(sha256sum "${path}" | awk '{print $1}')"
  elif [[ -e "${path}" || -L "${path}" ]]; then
    printf 'IDENTITY_UNSAFE_OR_NON_REGULAR label=%s path=%s\n' "${label}" "${path}"
  else
    printf 'IDENTITY_ABSENT label=%s path=%s\n' "${label}" "${path}"
  fi
}
...
show_text_file "${STATUS_ROOT}/gpu-resources-${EXECUTION_ID}-${JOB_ID}.csv" "gpu_resource_log"
show_text_file "${STATUS_ROOT}/cgroup-resources-${EXECUTION_ID}-${JOB_ID}.txt" "cgroup_resource_log"
show_text_file "${STATUS_ROOT}/resource-summary-${EXECUTION_ID}-${JOB_ID}.json" "resource_summary"
show_text_file "${STATUS_ROOT}/evaluation-start-${EXECUTION_ID}-${JOB_ID}.epoch" "evaluation_start"
show_text_file "${STATUS_ROOT}/evaluation-end-${EXECUTION_ID}-${JOB_ID}.epoch" "evaluation_end"
...
show_text_file "${RUN_ROOT}/evaluation/failure.json" "failure"
show_text_file "${RUN_ROOT}/evaluation/completion.marker.json" "completion_marker"
show_text_file "${RUN_ROOT}/verification/independent_verification.json" "independent_verification"
show_identity_file "${RUN_ROOT}/evaluation/predictions.npz" "predictions"
show_identity_file "${CHECKPOINT}" "epoch75_checkpoint"
show_identity_file "${RUN_ROOT}/inputs/DAILY_CAMELS_KNET_A38_JOB215801_TERMINAL_EVIDENCE_RECONSTRUCT_SEQ89.tar.gz" "reconstructed_evidence"
show_identity_file "${RUN_ROOT}/inputs/member_identity_verification.json" "member_identity_receipt"

printf 'SEQ100_A39_TERMINAL_DIAGNOSTIC_COMPLETE job_id=%s\n' "${JOB_ID}"
