...
for source, expected_id in ((sys.argv[1], sys.argv[2]), (sys.argv[3], sys.argv[4])):
    root = Path(source)
    manifest = json.loads((root / "bundle_manifest.json").read_text(encoding="utf-8"))
    active = root / manifest["active_config_member"]
    config = json.loads(active.read_text(encoding="utf-8"))
    policy = config.get("execution_policy", {})
    if (
        manifest.get("experiment_id") != expected_id
        or config.get("experiment_id") != expected_id
...
for experiment_id, initialization_mode in expected.items():
    ...
    if (
        ...
        or (
            gate.get("pytest_exit_code"),
            gate.get("remote_test_status"),
        ) not in {
            (0, "passed"),
