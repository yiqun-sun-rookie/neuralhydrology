...
ARCHIVE="${MAILBOX_ROOT}/outbox/kalmannet-daily-camels/DAILY_CAMELS_KNET_A38_JOB215801_TERMINAL_EVIDENCE_SEQ79.tar.gz"
...
[[ ! -e "$ARCHIVE" && ! -L "$ARCHIVE" ]] || {
  echo "sequence 79 terminal evidence archive already exists" >&2
  exit 57
}
...
SQUEUE_EXIT_CODE="$?"
set -e
printf '%s\n' 'SEQ79_A38_SQUEUE_BEGIN' "$SQUEUE_SNAPSHOT" 'SEQ79_A38_SQUEUE_END'
...
for path in "$LOG_OUT" "$LOG_ERR" "$PREFLIGHT_REPORT" "$CGROUP_RESOURCE_LOG" \
  ...
  if [[ -f "$path" && ! -L "$path" ]] && ! reserved_named_path "$path"; then
    printf 'SEQ79_A38_CONTENT_BEGIN path=%s size=%s sha256=%s\n' "$path" "$(stat -c '%s' "$path")" "$(sha256_file "$path")"
    sed -n '1,30000p' "$path"
    printf 'SEQ79_A38_CONTENT_END path=%s\n' "$path"
  else
    printf 'SEQ79_A38_CONTENT_ABSENT_OR_UNSAFE path=%s\n' "$path"
  fi
done
...
printf '%s\n' 'SEQ79_A38_GPU_RESOURCE_SUMMARY_BEGIN'
...
printf '%s\n' 'SEQ79_A38_GPU_RESOURCE_SUMMARY_END'
...
printf 'SEQ79_A38_HOST_MEMORY_RESOURCE_SUMMARY slurm_job_id=%s cgroup_memory_peak_status=unavailable slurm_training_step_max_rss_k=%s slurm_training_step_max_rss_bytes=%s slurm_training_step_max_vm_size_k=%s slurm_training_step_max_vm_size_bytes=%s\n' \
...
print("SEQ79_A38_INDEPENDENT_VALIDATION " + json.dumps(audit, sort_keys=True, separators=(",", ":")))
PY
