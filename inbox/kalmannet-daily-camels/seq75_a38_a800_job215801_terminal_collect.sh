...
SOURCE_REPLAY_EVIDENCE_SHA256="b06e2f86088798bb7c0374301e3871226708eb6d0a967dc84eadd7054a26e8e9"
SOURCE_REPLAY_EVIDENCE="${SOURCE_DIRECTORY}/artifacts/daily_camels_knet_a37_train4_terminal_evidence/job215699/run/replay_evidence.json"
ACTIVE_CONFIG_SHA256="05bace055154c9e88124f81434fc76a4e7a0e73d1ef4b8bf2635d7507b5d7d82"
ARCHIVE="${MAILBOX_ROOT}/outbox/kalmannet-daily-camels/DAILY_CAMELS_KNET_A38_JOB215801_TERMINAL_EVIDENCE_SEQ75.tar.gz"
...
SQUEUE_EXIT_CODE="$?"
set -e
printf '%s\n' 'SEQ75_A38_SQUEUE_BEGIN' "$SQUEUE_SNAPSHOT" 'SEQ75_A38_SQUEUE_END'
...
[[ -f "$SOURCE_REPLAY_EVIDENCE" && ! -L "$SOURCE_REPLAY_EVIDENCE" ]] || { echo "A37 source replay evidence is absent or symbolic" >&2; exit 63; }
[[ "$(sha256_file "$SOURCE_REPLAY_EVIDENCE")" == "$SOURCE_REPLAY_EVIDENCE_SHA256" ]] || { echo "A37 source replay evidence SHA-256 differs" >&2; exit 63; }

printf '%s\n' 'SEQ75_A38_SACCT_BEGIN'
...
for path in "$LOG_OUT" "$LOG_ERR" "$PREFLIGHT_REPORT" "$CGROUP_RESOURCE_LOG" \
  ...
  if [[ -f "$path" && ! -L "$path" ]] && ! reserved_named_path "$path"; then
    printf 'SEQ75_A38_CONTENT_BEGIN path=%s size=%s sha256=%s\n' "$path" "$(stat -c '%s' "$path")" "$(sha256_file "$path")"
    sed -n '1,30000p' "$path"
    printf 'SEQ75_A38_CONTENT_END path=%s\n' "$path"
  else
    printf 'SEQ75_A38_CONTENT_ABSENT_OR_UNSAFE path=%s\n' "$path"
  fi
done

[[ -f "$GPU_RESOURCE_LOG" && ! -L "$GPU_RESOURCE_LOG" ]] || { echo "A38 GPU resource log is absent or symbolic" >&2; exit 66; }
printf '%s\n' 'SEQ75_A38_GPU_RESOURCE_SUMMARY_BEGIN'
GPU_RESOURCE_SUMMARY="$(python - "$GPU_RESOURCE_LOG" "$TRAINING_STEP_START" "$TRAINING_STEP_END" "$TRAINING_STEP_ELAPSED_SECONDS" <<'GPU_PY'
import csv
...
step_start = dt.datetime.strptime(sys.argv[2], "%Y-%m-%dT%H:%M:%S")
step_end = dt.datetime.strptime(sys.argv[3], "%Y-%m-%dT%H:%M:%S")
...
if (
    rows[0][0] > step_start + dt.timedelta(seconds=10)
    or rows[-1][0] < step_end - dt.timedelta(seconds=10)
    or coverage_seconds < max(0, elapsed_seconds - 20)
):
    raise SystemExit("GPU resource log does not cover the completed training step")
print(
...
printf '%s\n' 'SEQ75_A38_GPU_RESOURCE_SUMMARY_END'
...
printf 'SEQ75_A38_CGROUP_RESOURCE_SUMMARY slurm_job_id=%s memory_peak_bytes=%s\n' "$JOB_ID" "$CGROUP_MEMORY_PEAK_BYTES"

python - "$RUN_DIRECTORY" "$PREFLIGHT_REPORT" "$SOURCE_RESULT_SUMMARY" "$EXPERIMENT_ID" "$SOURCE_CHECKPOINT_SHA256" "$SOURCE_EPOCH_HISTORY_SHA256" "$SOURCE_EPOCH40_PARAMETER_SHA256" "$SOURCE_CONFIGURATION_SHA256" "$SOURCE_IDENTITY_SHA256" "$SOURCE_RESULT_SUMMARY_SHA256" "$SOURCE_EVIDENCE_ARCHIVE_SHA256" "$ACTIVE_CONFIG_SHA256" "$JOB_ID" "$SOURCE_REPLAY_EVIDENCE" "$SOURCE_DIRECTORY" <<'PY'
import copy
import hashlib
import json
import math
...
job_id = sys.argv[13]
source_replay_path = Path(sys.argv[14])
source_directory = Path(sys.argv[15])
...
if not source_replay_path.is_file() or source_replay_path.is_symlink() or sha(source_replay_path) != "b06e2f86088798bb7c0374301e3871226708eb6d0a967dc84eadd7054a26e8e9":
    raise SystemExit("registered A37 source replay evidence differs")
source_replay = json.loads(source_replay_path.read_text(encoding="utf-8"))
...
expected_replay = copy.deepcopy(source_replay)
expected_replay["experiment_id"] = experiment_id
expected_replay["identity_sha256"] = actual_identity_sha
expected_selection_path = str(
    source_directory
    / "results/tukf06_full_diagonal_search_head_to_head_v1/selection/basin_09035800.json"
)
for expected_window in expected_replay["windows"].values():
    expected_window["ukf"]["metadata"]["selection_path"] = expected_selection_path
if replay_evidence != expected_replay:
    raise SystemExit("complete UKF and zero-gain replay evidence differs from the registered A37 source")
...
audit = {
    ...
    "source_replay_evidence_sha256": sha(source_replay_path),
...
print("SEQ75_A38_INDEPENDENT_VALIDATION " + json.dumps(audit, sort_keys=True, separators=(",", ":")))
PY
