...
EXECUTION_ID="DAILY_CAMELS_KNET_EPOCH75_SINGLE_BASIN_FORMAL_EVALUATION_A39_A800_EVAL4_SEQ94"
SUBMISSION_TOKEN="00ae298e699c5ea81bbb0cc25886f94d30fc8e32a1f23c30202d88f3a5a1bb3f"
JOB_NAME="daily-knet-a39-s94"
SUBMISSION_COMMENT="${EXECUTION_ID}"
RUN_ROOT="/data1/home/sunyiq/kalmannet_daily_camels_knet_a39_formal_evaluation4_20260831"
SOURCE_ROOT="${RUN_ROOT}/source_A39_formal_evaluation_seq94"
...
PAYLOAD_ROOT="${MAILBOX_ROOT}/payload/${CHANNEL}/a39_epoch75_single_basin_formal_evaluation_eval4_seq94_20260831"
...
ARCHIVE_SIZE="229971"
OUTER_MANIFEST_SHA256="9678376298f9d91addc3cf98dfac0b212142e61b6dbd45e55a1a699578e5f0e8"
OUTER_MANIFEST_SIZE="2627"
INTERNAL_MANIFEST_SHA256="d88b2b1e4579eb2ad6adbdf3160f2c3c1fe6d01a99aae7bec82c99dc2705b7f8"
...
expected = {
    ...
    "prior_failed_job_id": "216847",
    "prior_failure_class": "recoverable_preflight_gpu_count_field_misinterpretation",
    "mailbox_sequence": 94,
}
for key, expected_value in expected.items():
    if payload.get(key) != expected_value:
...
recover_only() {
  ...
  temporary_directory="$(mktemp -d /tmp/a39-seq94-recover.XXXXXX)"
  ...
  if [[ -f "${JOB_ID_FILE}" && ! -L "${JOB_ID_FILE}" ]]; then
    ...
    echo "SEQ94_A39_RECOVERED_RECORDED job_id=${existing_job_id}"
    return 0
  fi
  ...
  if [[ -f "${BOUND_JSON}" && ! -L "${BOUND_JSON}" ]]; then
    ...
    echo "SEQ94_A39_RECOVERED_BOUND job_id=${bound_job_id}"
    return 0
  fi
  ...
  echo "SEQ94_A39_RECOVERED_AFTER_AMBIGUOUS_RECEIPT job_id=${recovered_ids[0]}"
}
...
PRE_QUERY="$(mktemp -d /tmp/a39-seq94-pre.XXXXXX)"
...
if [[ "${SBATCH_EXIT}" == 0 && "${RAW_RECEIPT}" =~ ^[0-9]+(\;[^[:space:]]+)?$ && "${JOB_ID}" =~ ^[0-9]+$ ]]; then
  ...
  echo "SEQ94_A39_FORMAL_EVALUATION_SUBMITTED experiment_id=${EXPERIMENT_ID} execution_id=${EXECUTION_ID} job_id=${JOB_ID} archive_sha256=${ARCHIVE_SHA256} manifest_sha256=${OUTER_MANIFEST_SHA256} slurm_sha256=${SLURM_SHA256} preflight_sha256=${PREFLIGHT_SHA256} result90_sha256=${RESULT90_SHA256} result91_sha256=${RESULT91_SHA256} result92_sha256=${RESULT92_SHA256} result93_sha256=${RESULT93_SHA256} reconstructed_sha256=${RECONSTRUCTED_SHA256} receipt_sha256=${RECEIPT_SHA256} root=${RUN_ROOT} source_root=${SOURCE_ROOT}"
  exit 0
fi
