...
FAILED_ARCHIVE="${MAILBOX_ROOT}/outbox/kalmannet-daily-camels/DAILY_CAMELS_KNET_A38_JOB215801_TERMINAL_EVIDENCE_SEQ75.tar.gz"
ARCHIVE="${MAILBOX_ROOT}/outbox/kalmannet-daily-camels/DAILY_CAMELS_KNET_A38_JOB215801_TERMINAL_EVIDENCE_SEQ76.tar.gz"
...
[[ ! -e "$FAILED_ARCHIVE" && ! -L "$FAILED_ARCHIVE" ]] || {
  echo "failed sequence 75 archive unexpectedly exists" >&2
  exit 57
}
[[ ! -e "$ARCHIVE" && ! -L "$ARCHIVE" ]] || {
  echo "sequence 76 terminal evidence archive already exists" >&2
  exit 57
}
...
SQUEUE_EXIT_CODE="$?"
set -e
printf '%s\n' 'SEQ76_A38_SQUEUE_BEGIN' "$SQUEUE_SNAPSHOT" 'SEQ76_A38_SQUEUE_END'
...
for path in "$LOG_OUT" "$LOG_ERR" "$PREFLIGHT_REPORT" "$CGROUP_RESOURCE_LOG" \
  ...
  if [[ -f "$path" && ! -L "$path" ]] && ! reserved_named_path "$path"; then
    printf 'SEQ76_A38_CONTENT_BEGIN path=%s size=%s sha256=%s\n' "$path" "$(stat -c '%s' "$path")" "$(sha256_file "$path")"
    sed -n '1,30000p' "$path"
    printf 'SEQ76_A38_CONTENT_END path=%s\n' "$path"
  else
    printf 'SEQ76_A38_CONTENT_ABSENT_OR_UNSAFE path=%s\n' "$path"
  fi
done

[[ -f "$GPU_RESOURCE_LOG" && ! -L "$GPU_RESOURCE_LOG" ]] || { echo "A38 GPU resource log is absent or symbolic" >&2; exit 66; }
printf '%s\n' 'SEQ76_A38_GPU_RESOURCE_SUMMARY_BEGIN'
...
printf '%s\n' 'SEQ76_A38_GPU_RESOURCE_SUMMARY_END'
...
printf 'SEQ76_A38_CGROUP_RESOURCE_SUMMARY slurm_job_id=%s memory_peak_bytes=%s\n' "$JOB_ID" "$CGROUP_MEMORY_PEAK_BYTES"
...
print("SEQ76_A38_INDEPENDENT_VALIDATION " + json.dumps(audit, sort_keys=True, separators=(",", ":")))
PY
