...
printf 'SEQ97_A39_TERMINAL_DIAGNOSTIC experiment_id=%s execution_id=%s job_id=%s\n' \
...
for directory in \
  "${RUN_ROOT}" \
  "${RUN_ROOT}/source_A39_formal_evaluation_seq94" \
  "${RUN_ROOT}/logs" \
  "${STATUS_ROOT}" \
  "${STATUS_ROOT}/locks" \
  "${RUN_ROOT}/inputs" \
...
printf 'SEQ97_A39_TERMINAL_DIAGNOSTIC_COMPLETE job_id=%s\n' "${JOB_ID}"
