...
printf 'SEQ93_A39_TERMINAL_DIAGNOSTIC experiment_id=%s execution_id=%s job_id=%s\n' \
...
for directory in \
  "${RUN_ROOT}" \
  "${RUN_ROOT}/source_A39_formal_evaluation_seq91" \
  "${RUN_ROOT}/logs" \
  "${STATUS_ROOT}" \
  "${RUN_ROOT}/evaluation" \
...
printf 'SEQ93_A39_TERMINAL_DIAGNOSTIC_COMPLETE job_id=%s\n' "${JOB_ID}"
