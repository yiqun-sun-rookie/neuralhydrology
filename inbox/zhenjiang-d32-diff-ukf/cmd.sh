...
echo "=== SCHEDULER_IDENTITY ==="
scontrol show job -o "${JOB_ID}" || true
echo "=== ACCOUNTING ==="
sacct -j "${JOB_ID}" -P \
  --format=JobIDRaw,JobName,Partition,State,ExitCode,Elapsed,Start,End,NodeList || true
...
if [ -f "${OUT_LOG}" ]; then tail -n 200 "${OUT_LOG}"; else echo absent; fi
echo "=== STANDARD_ERROR_TAIL ==="
if [ -f "${ERR_LOG}" ]; then tail -n 200 "${ERR_LOG}"; else echo absent; fi
...
for seed in (17, 29, 43):
    ...
    if (
        ...
        raise SystemExit(
            "revision-three precheck semantic gate failed for seed %d" % seed
        )
    print(
        "REVISION_THREE_PRECHECK_SEED_GATE=PASS "
        "seed=%d samples=%d checkpoint=%s normalization=%s"
        % (
            seed,
            worker["processed_base_sample_count"],
            checkpoint["best_checkpoint_sha256"],
            normalization["verification_method"],
        )
    )
print("REVISION_THREE_THREE_SEED_PRECHECK_GATE=PASS")
PY
fi
