#!/bin/bash
# kalmannet-daily-perbasin sequence=114: READ-ONLY observation of training job 225569 (basin 03076600, launch seq 113).
set -o pipefail
echo "channel=kalmannet-daily-perbasin sequence=114 purpose=read-only-observe-train-03076600-job225569"
...
RUN=$ROOT/runs/DAILY_CAMELS_KNET_PER_BASIN_21_DEVELOPMENT_V2_20260908_BASIN_03076600_A40_TRAIN1_SEQ113
JOB=225569
...
case "$STATE" in
  COMPLETED|FAILED|TIMEOUT|NODE_FAIL|OUT_OF_MEMORY|CANCELLED*)
    ...
    emit_file "$REQ/submission_seq113.receipt.json"
