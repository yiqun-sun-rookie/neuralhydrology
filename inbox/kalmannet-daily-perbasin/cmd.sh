#!/bin/bash
# kalmannet-daily-perbasin sequence=153: READ-ONLY observation (zero_gain). No sbatch, no scancel, no writes.
set -o pipefail
echo "channel=kalmannet-daily-perbasin sequence=153 purpose=observe-zero_gain"
...
timeout 30s sacct -n -P -j 226291 --format=JobIDRaw,JobName%32,State,ExitCode,ElapsedRaw,AllocCPUS,AllocTRES%40,NodeList,Submit,Start,End 2>&1
echo "=== QUEUE ==="
...
echo "=== REQUEST zero_gain_probe_seq152 ==="
for f in $ROOT/runtime/zero_gain_probe_seq152/slurm-*.stdout; do [ -f "$f" ] && tail_file "$f" 40; done; true
for f in $ROOT/runtime/zero_gain_probe_seq152/slurm-*.stderr; do [ -f "$f" ] && tail_file "$f" 20; done; true
emit_json $ROOT/runtime/zero_gain_probe_seq152/logs/pool_result.json
for f in $ROOT/runtime/zero_gain_probe_seq152/logs/*.stderr; do [ -f "$f" ] && [ -s "$f" ] && tail_file "$f" 15; done; true
...
echo "=== FULL_STDOUT zero_gain_probe_seq152 ==="; for f in $ROOT/runtime/zero_gain_probe_seq152/slurm-*.stdout; do [ -f "$f" ] && { echo "FULL_BEGIN path=$f bytes=$(stat -c %s "$f")"; cat "$f"; echo "FULL_END path=$f"; }; done; true
