#!/bin/bash
# kalmannet-daily-perbasin sequence=125: READ-ONLY observation of training job 225766 (basin 01440400, launch seq 124).
set -o pipefail
echo "channel=kalmannet-daily-perbasin sequence=125 purpose=read-only-observe-train-01440400-job225766"
...
RUN=$ROOT/runs/DAILY_CAMELS_KNET_PER_BASIN_21_DEVELOPMENT_V2_20260908_BASIN_01440400_A40_TRAIN1_SEQ124
JOB=225766
...
case "$STATE" in
  COMPLETED|FAILED|TIMEOUT|NODE_FAIL|OUT_OF_MEMORY|CANCELLED*)
    ...
    emit_file "$REQ/submission_seq124.receipt.json"
