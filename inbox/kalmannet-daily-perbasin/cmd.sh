#!/bin/bash
# kalmannet-daily-perbasin sequence=106: READ-ONLY observation of training job 225494 (basin 01487000, launch seq 105).
set -o pipefail
echo "channel=kalmannet-daily-perbasin sequence=106 purpose=read-only-observe-train-01487000-job225494"
...
RUN=$ROOT/runs/DAILY_CAMELS_KNET_PER_BASIN_21_DEVELOPMENT_V2_20260908_BASIN_01487000_A40_TRAIN1_SEQ105
JOB=225494
...
case "$STATE" in
  COMPLETED|FAILED|TIMEOUT|NODE_FAIL|OUT_OF_MEMORY|CANCELLED*)
    ...
    emit_file "$REQ/submission_seq105.receipt.json"
