...
printf '%s\n' 'channel=kalmannet-daily-perbasin sequence=58 purpose=readonly-first-basin-training-progress'
...
emit({'observed_at_utc': datetime.datetime.now(datetime.timezone.utc).isoformat(), 'request_sequence': 58, 'job_id': job_id, 'execution_id': execution_id, 'submission_receipt_sha256': hashlib.sha256(receipt_bytes).hexdigest(), 'run_directory': str(run_directory)})
...
emit({'status': 'READONLY_TRAINING_PROGRESS_OBSERVED', 'request_sequence': 58, 'job_id': job_id, 'compute_submissions': 0, 'task_file_writes': 0, 'checkpoint_content_reads': 0, 'formal_evaluation_access_count': 0})
PY_PROGRESS
