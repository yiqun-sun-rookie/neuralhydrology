#!/bin/bash
# kalmannet-daily-perbasin sequence=112: READ-ONLY observation of training job 225568 (basin 06803510, launch seq 111).
set -o pipefail
echo "channel=kalmannet-daily-perbasin sequence=112 purpose=read-only-observe-train-06803510-job225568"
...
RUN=$ROOT/runs/DAILY_CAMELS_KNET_PER_BASIN_21_DEVELOPMENT_V2_20260908_BASIN_06803510_A40_TRAIN1_SEQ111
JOB=225568
...
case "$STATE" in
  COMPLETED|FAILED|TIMEOUT|NODE_FAIL|OUT_OF_MEMORY|CANCELLED*)
    ...
    emit_file "$REQ/submission_seq111.receipt.json"
