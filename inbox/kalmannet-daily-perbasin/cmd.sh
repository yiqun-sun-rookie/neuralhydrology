...
printf '%s\n' 'channel=kalmannet-daily-perbasin sequence=65 purpose=readonly-second-basin-terminal-evidence-and-shared-resource-state'
...
execution_id = 'DAILY_CAMELS_KNET_PER_BASIN_PILOT_08070200_V2_20260902_A44_HISTORYFIX1_A800_TRAIN1_SEQ62'
run_directory = root / 'runs' / execution_id
launch = root / 'node_recovery_20260907/train_08070200_historyfix1_seq62'
status = root / 'status'
job_id = '223689'
...
emit({'section': 'IDENTITY', 'observed_at_utc': datetime.datetime.now(datetime.timezone.utc).isoformat(), 'request_sequence': 65, 'execution_id': execution_id, 'job_id': job_id, 'run_directory': str(run_directory)})
...
emit({'status': 'READONLY_TERMINAL_EVIDENCE_AND_RESOURCE_STATE_OBSERVED', 'request_sequence': 65, 'job_id': job_id, 'compute_submissions': 0, 'task_file_writes': 0, 'checkpoint_content_reads': 0, 'prediction_array_reads': 0, 'formal_evaluation_access_count': 0})
PY_EVIDENCE
