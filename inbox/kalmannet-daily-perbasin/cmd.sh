#!/bin/bash
# kalmannet-daily-perbasin sequence=129: READ-ONLY observation of training job 225768 (basin 03049000, launch seq 128).
set -o pipefail
echo "channel=kalmannet-daily-perbasin sequence=129 purpose=read-only-observe-train-03049000-job225768"
...
RUN=$ROOT/runs/DAILY_CAMELS_KNET_PER_BASIN_21_DEVELOPMENT_V2_20260908_BASIN_03049000_A40_TRAIN1_SEQ128
JOB=225768
...
case "$STATE" in
  COMPLETED|FAILED|TIMEOUT|NODE_FAIL|OUT_OF_MEMORY|CANCELLED*)
    ...
    emit_file "$REQ/submission_seq128.receipt.json"
