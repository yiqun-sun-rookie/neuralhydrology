#!/bin/bash
# kalmannet-daily-perbasin sequence=116: READ-ONLY observation of training job 225584 (basin 12175500, launch seq 115).
set -o pipefail
echo "channel=kalmannet-daily-perbasin sequence=116 purpose=read-only-observe-train-12175500-job225584"
...
RUN=$ROOT/runs/DAILY_CAMELS_KNET_PER_BASIN_21_DEVELOPMENT_V2_20260908_BASIN_12175500_A40_TRAIN1_SEQ115
JOB=225584
...
case "$STATE" in
  COMPLETED|FAILED|TIMEOUT|NODE_FAIL|OUT_OF_MEMORY|CANCELLED*)
    ...
    emit_file "$REQ/submission_seq115.receipt.json"
