...
printf '%s\n' 'channel=kalmannet-daily-perbasin sequence=69 purpose=readonly-third-basin-terminal-evidence-and-shared-resource-state'
...
emit({'section': 'IDENTITY', 'observed_at_utc': datetime.datetime.now(datetime.timezone.utc).isoformat(), 'request_sequence': 69, 'execution_id': execution_id, 'job_id': job_id, 'run_directory': str(run_directory)})
...
emit({'status': 'READONLY_TERMINAL_EVIDENCE_AND_RESOURCE_STATE_OBSERVED', 'request_sequence': 69, 'job_id': job_id, 'compute_submissions': 0, 'task_file_writes': 0, 'checkpoint_content_reads': 0, 'prediction_array_reads': 0, 'formal_evaluation_access_count': 0})
PY_EVIDENCE
