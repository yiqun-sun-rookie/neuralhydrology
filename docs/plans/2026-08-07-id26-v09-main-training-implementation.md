...
### S4 串行编排器 `run_formal_training_v09.py` — 已完成

**设计变更（2026-08-07，用户批准）：不使用一次性授权收据。**
它只能消费一次，而 24 次训练约 34 小时、跨多个调度作业，一次中断就会烧掉收据并困住
所有已完成的运行。科学保证不在这把锁上，而在冻结协议、封存并哈希的输入、冻结的运行顺序，
以及"全部预测封存后才抽取"的秘密留出集——这些一条没动。

进度不由编排器记账，而是每次启动时**从已封存的运行目录推导**：目录里的封印哈希与磁盘
字节仍然吻合才算完成。半写状态（`.building` / `.failed`）一律停下交人判断，绝不自动清理
或从半截续跑。父目录按模型族分 `classic/` `capacity/` `continuous/`。
...
- [x] S4 串行编排器（可断点续跑，无一次性锁）
- [x] S5a 状态诊断（预注册面板/五列/汇总，已实现）
- [ ] S5b 24 次总审核 + 总封存（未做）
