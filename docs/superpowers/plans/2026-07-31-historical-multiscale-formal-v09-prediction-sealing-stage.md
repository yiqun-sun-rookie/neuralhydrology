...
- 前置证据必须同时存在并通过：正式输入封存、输入产物独立审核、可信训练目标来源独立审核、
  旧参考函数桥接审核、严格嵌套封存、严格嵌套独立审核、24次主训练总封存和训练独立审核。
...
严格嵌套运行封存、严格嵌套外部审核报告、训练总封存、训练外部审核报告、24个第30轮检查点和提交配置。
...
- 输入产物外部审核、可信训练目标来源外部审核、旧参考函数桥接审核、严格嵌套外部审核和训练外部审核哈希；
