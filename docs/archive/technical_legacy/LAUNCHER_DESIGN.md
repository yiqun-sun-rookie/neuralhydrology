...
> Historical design note: this document describes a proposed unified `train.py` wrapper.
> Current canonical runner in this repository is `python -m neuralhydrology.nh_run ...`.
...
| `--config-dir` | path | src/test_data/configs | Directory scanned by `--list-configs` |
...
### Basic
```
python train.py -c src/test_data/configs/test_config.yml --gpu 0
```
### Auto Optimize
```
...
### List Configs
```
python train.py --list-configs --config-dir src/test_data/configs
```
