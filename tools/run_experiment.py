...
Usage:
    ...
    python tools/run_experiment.py quick --site namou_kuwei --type ar --lead 1
    
    # Run a single experiment
    python tools/run_experiment.py train --config src/namou_kuwei/configs/no_leak/03_with_ar/rain_ar_LT1h.yml
    
    # Evaluate existing run
    python tools/run_experiment.py evaluate --run-dir results/namou_kuwei/L3_Rain_AR_LT1h_2025_1204_...
...
def run_train(config_path: str, device: str = None):
    ...
    result = subprocess.run(
        [sys.executable, "tools/validate_config.py", config_path],
...
def run_quick(site: str, config_type: str, lead: int, predict_steps: int = 1, device: str = None):
    ...
    from tools.gen_config import generate_config, save_config
    from pathlib import Path
    ...
    output_dir = Path("src") / site / "configs" / "generated"
    ...
    result = subprocess.run(
        [sys.executable, "tools/validate_config.py", str(output_path)],
        capture_output=True, text=True
    )
    print(result.stdout)
    if result.returncode != 0:
