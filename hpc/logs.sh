...
#
# 用法:
#   ./hpc/logs.sh                     # 列出所有任务日志概览
#   ./hpc/logs.sh list                # 同上
#   ./hpc/logs.sh <task>              # 列出指定任务的日志
#   ./hpc/logs.sh <task> <job_id>     # 查看指定作业的日志
#   ./hpc/logs.sh tail <task>         # 实时查看最新日志
#   ./hpc/logs.sh clean <task>        # 清理指定任务的旧日志
#   ./hpc/logs.sh clean all           # 清理所有旧日志
#
# 示例:
#   ./hpc/logs.sh mamba_quick
#   ./hpc/logs.sh mamba_quick 12345
#   ./hpc/logs.sh tail caravan
#   ./hpc/logs.sh clean mamba_quick
# =============================================================================
...
list_all() {
    ...
    echo ""
    echo -e "${YELLOW}用法: ./hpc/logs.sh <task_name> [job_id]${NC}"
}
...
list_task() {
    ...
    echo ""
    echo -e "${YELLOW}查看日志: ./hpc/logs.sh $task <job_id>${NC}"
    echo -e "${YELLOW}实时日志: ./hpc/logs.sh tail $task${NC}"
}
...
case "$ACTION" in
    ...
    tail)
        if [ -z "$TASK" ]; then
            echo -e "${RED}[ERROR] 请指定任务名称: ./hpc/logs.sh tail <task>${NC}"
            exit 1
        fi
        tail_logs "$TASK"
        ;;
    clean)
        if [ -z "$TASK" ]; then
            echo -e "${RED}[ERROR] 请指定任务名称或 'all': ./hpc/logs.sh clean <task|all>${NC}"
            exit 1
        fi
        clean_logs "$TASK"
        ;;
    *)
        ...
        if [ -d "$LOG_BASE/$ACTION" ]; then
            ...
            echo ""
            echo "用法:"
            echo "  ./hpc/logs.sh                   # 列出所有任务"
            echo "  ./hpc/logs.sh <task>            # 列出任务日志"
            echo "  ./hpc/logs.sh <task> <job_id>   # 查看作业日志"
            echo "  ./hpc/logs.sh tail <task>       # 实时日志"
            echo "  ./hpc/logs.sh clean <task|all>  # 清理旧日志"
            exit 1
        fi
        ;;
esac
