...
if [ -d "configs" ]; then
    ...
    echo "配置文件已复制到 $CONFIGS_DIR"
else
    echo "警告: 未找到configs目录"
fi

# 复制SLURM作业脚本
if [ -d "slurm_jobs" ]; then
    cp -r slurm_jobs/* $PROJECT_DIR/slurm_jobs/
    echo "SLURM作业脚本已复制到 $PROJECT_DIR/slurm_jobs"
else
    echo "警告: 未找到slurm_jobs目录"
fi

# 更新配置文件中的邮箱地址
echo "更新配置文件中的邮箱地址..."
find $PROJECT_DIR/slurm_jobs -name "*.slurm" -exec sed -i "s/your_email@hhu.edu.cn/$USER_EMAIL/g" {} \;
...
if [ -d "$DATA_DIR" ] && [ "$(ls -A $DATA_DIR)" ]; then
    ...
    echo "数据下载命令示例:"
    echo "  python scripts/download_camels_us.py --data_dir $DATA_DIR"
fi
...
chmod +x $PROJECT_DIR/slurm_jobs/*.slurm
...
echo "下一步操作:"
echo "1. 检查数据是否已正确上传"
echo "2. 运行快速测试: sbatch $PROJECT_DIR/slurm_jobs/hpc_quick_test.slurm"
echo "3. 如果测试通过，提交完整训练: sbatch $PROJECT_DIR/slurm_jobs/hpc_full_training.slurm"
