...
def copy_model_files(run_dir, model_dir):
    ...
    # 需要复制的文件
    files_to_copy = [
        "model_epoch050.pt",  # 最终模型权重
        "config.yml",         # 配置文件
        "output.log",         # 训练日志
        "train_data/train_data_scaler.yml",  # 数据标准化参数
    ]
...
def copy_visualization_files(run_dir, model_dir):
    ...
    if img_log_src.exists():
        ...
        print(f"  ✅ 复制可视化文件: {len(list(img_log_dst.glob('*.png')))} 个图片")
    else:
        print("  ⚠️ 未找到可视化文件")
...
```
{model_name}/
├── model_epoch050.pt              # 最终训练模型权重
├── config.yml                     # 训练配置文件
├── train_data_scaler.yml          # 数据标准化参数
├── output.log                     # 完整训练日志
├── model_info.yml                 # 模型基本信息
├── data_splits/                   # 数据划分文件
│   ├── full_674_basins_train_basins.txt
│   ├── full_674_basins_val_basins.txt
│   └── full_674_basins_test_basins.txt
├── visualizations/                # 训练可视化图片
│   └── *.png
├── MODEL_DOCUMENTATION.md         # 详细技术文档
...
- [使用指南](USAGE_GUIDE.md) - 模型使用方法和代码示例
...
def main():
    ...
    if len(sys.argv) < 2:
        print("❌ 请提供运行目录路径")
        print("使用方法: python scripts/save_model.py <run_directory>")
        return
