...
此脚本专门用于完整数据集（674个流域）的训练，基于 simple_train.py 进行优化。

功能：
1. 检查 neuralhydrology_gpu conda 环境是否存在
2. 读取 YAML 配置文件（默认 configs/full_training/full_training.yml）
3. 检查 GPU 可用性和内存
4. 在指定 conda 环境中启动训练
5. 支持断点续训功能
6. 训练进度监控和资源使用统计
...
使用方式：
    # 新训练（完整数据集）
    python scripts/full_train.py --config configs/full_training/full_training.yml --gpu 0
    
    # 断点续训（自动选择最新运行）
    python scripts/full_train.py --resume
    
    # 从指定运行目录恢复训练
    python scripts/full_train.py --resume --run-dir runs/full_training_2025_1024_1631_ep3
    
    # 从指定epoch恢复训练
    python scripts/full_train.py --resume --run-dir runs/full_training_2025_1024_1631_ep3 --epoch 2
    
    # 监控模式（显示详细进度）
    python scripts/full_train.py --monitor --config configs/full_training/full_training.yml
...
DEFAULT_CONFIG = "configs/full_training/full_training.yml"
...
def backup_important_files():
    ...
    # 备份配置文件
    config_files = [
        "configs/full_training/full_training.yml",
    ...
    if len(sys.argv) == 1:
        ...
        try:
            ...
            if use_interactive in ['y', 'yes']:
                ...
                print("\n[INFO] 使用命令行模式")
                print("\n使用方法:")
                print("  python scripts/full_train.py --config configs/full_training/full_training_optimized.yml --gpu 0")
                print("  python scripts/full_train.py --resume  # 继续训练")
                print("  python scripts/full_train.py --interactive  # 启用交互式菜单")
                return True
        except KeyboardInterrupt:
            print("\n[INFO] 操作已取消")
            return True
    ...
    if args.interactive:
        while True:
            ...
            if choice == 1:  # 新训练
                ...
                print("\n[INFO] 启动训练监控...")
                try:
                    subprocess.run(["python", "scripts/monitor_training.py", "--verbose"], check=True)
                ...
                print("\n[INFO] 生成训练报告...")
                try:
                    subprocess.run(["python", "scripts/monitor_training.py", "--report"], check=True)
                except subprocess.CalledProcessError:
                    print("[ERROR] 报告生成失败")
                except FileNotFoundError:
                    print("[ERROR] 找不到监控脚本")
