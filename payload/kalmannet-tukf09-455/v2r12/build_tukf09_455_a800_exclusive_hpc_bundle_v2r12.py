"""Build, inspect, and safely extract the TUKF09 455-basin A800-exclusive HPC v2r12 bundle.
...
DEFAULT_BUNDLE_OUTPUT = ROOT / "artifacts" / "tukf09_455_a800_exclusive_hpc_bundle_v2r12"
ARCHIVE_NAME = "tukf09_455_basin_zero_validation_target_variance_revision_v1_hpc_execution_a800_exclusive_v2r12_formal_training.tar.gz"
INTERNAL_MANIFEST_NAME = "bundle_manifest.json"
INTERNAL_SCHEMA = "tukf09_455_a800_exclusive_training_hpc_bundle_manifest_v2r12"
OUTER_SCHEMA = "tukf09_455_a800_exclusive_training_hpc_archive_manifest_v2r12"
...
LOCAL_FILTER_INSTALLATION_FINAL_SHA256 = (
    "37a2989c41ed767443629f49aadbbe0237772aa72822ad87f23265701fa04621"
)
...
CONFIG_RELATIVES = (
    ...
    "configs/tukf09_455_basin_zero_validation_target_variance_hpc_execution_a800_exclusive_v2r12.json",
    "configs/tukf09_455_basin_zero_validation_target_variance_revision_v1.json",
)
HPC_WRAPPER_RELATIVES = (
    "hpc/tukf09_455_basin_revision_a800_exclusive_v2r12/allocation_probe.slurm",
    "hpc/tukf09_455_basin_revision_a800_exclusive_v2r12/download_runtime_inputs_login.sh",
    "hpc/tukf09_455_basin_revision_a800_exclusive_v2r12/probe_gpu.slurm",
    "hpc/tukf09_455_basin_revision_a800_exclusive_v2r12/psutil-source.lock",
    "hpc/tukf09_455_basin_revision_a800_exclusive_v2r12/runtime-binary.lock",
    "hpc/tukf09_455_basin_revision_a800_exclusive_v2r12/stage_and_train.py",
    "hpc/tukf09_455_basin_revision_a800_exclusive_v2r12/submit_training_gpu.slurm",
    "hpc/tukf09_455_basin_revision_a800_exclusive_v2r12/verify_result.py",
)
BUILDER_RELATIVE = "scripts/build_tukf09_455_a800_exclusive_hpc_bundle_v2r12.py"
...
def _atomic_write_bytes(path: Path, content: bytes, *, refuse_different: bool) -> bool:
    destination = _absolute_io_path(path, force_windows_long=(os.name == "nt"))
...
def _validate_config_anchors(
    root: Path,
    admission: Mapping[str, Any],
    *,
    validate_optional_local_filter_installation: bool = True,
) -> None:
    project_root = Path(root)
    config_sources = {
    ...
    hpc_runtime = hpc.get("required_hpc_runtime", {})
    hpc_slurm = hpc.get("slurm", {})
    hpc_transition = hpc.get("runtime_transition", {})
    hpc_retry = hpc.get("technical_retry", {})
    source_capsule = hpc.get("training_source_capsule", {})
    hpc_remote_layout = hpc.get("remote_layout", {})
    hpc_data_staging = hpc.get("data_staging", {})
    offline_inputs = hpc.get("offline_runtime_inputs", {})
    offline_records = offline_inputs.get("expected_files", {})
    offline_values = (
        list(offline_records.values()) if isinstance(offline_records, Mapping) else []
    )
    offline_binary_records = [
        record
        for record in offline_values
        if isinstance(record, Mapping) and record.get("kind") == "binary_wheel"
    ]
    offline_source_records = [
        record
        for record in offline_values
        if isinstance(record, Mapping) and record.get("kind") == "source_archive"
    ]
    binary_lock_path = (
        project_root
        / "hpc/tukf09_455_basin_revision_a800_exclusive_v2r12/runtime-binary.lock"
    ...
    binary_lock_hashes = set(
        re.findall(r"--hash=sha256:([0-9a-f]{64})(?:\s|$)", binary_lock_path.read_text("utf-8"))
    )
    source_lock_hashes = set(
        re.findall(r"--hash=sha256:([0-9a-f]{64})(?:\s|$)", source_lock_path.read_text("utf-8"))
    )
    private_runtime = hpc.get("private_runtime_provenance", {})
    private_distributions = private_runtime.get("expected_distribution_versions", {})
    ...
    if (
        hpc.get("schema_version") != "tukf09_455_basin_hpc_execution_a800_exclusive_v2r12"
        or hpc.get("experiment_id") != EXPERIMENT_ID
        or hpc.get("execution_route", {}).get("formal_evaluation_access") is not False
        or hpc.get("execution_route", {}).get("neural_model_parallelism") != 8
        or hpc_transition.get("scientific_contract_change_allowed") is not False
        or hpc_transition.get("bitwise_equivalence_to_rtx3090_claimed") is not False
        or hpc_retry
        != {
            "revision": "v2r12",
            "supersedes_revision": "v2r11",
            "v2r11_was_never_built_deployed_or_submitted": True,
            "superseded_remote_root": "/data1/home/sunyiq/kalmannet_tukf09_455_basin_zero_validation_target_variance_revision_v1_a800_exclusive_v2r10_20260904",
            "superseded_training_job_id": 220663,
            "superseded_training_job_state": "FAILED",
            "superseded_training_exit_code": "1:0",
            "superseded_training_elapsed": "15:04:31",
            "superseded_training_node": "ngu203",
            "confirmed_failure_traceback_line": "run_tukf09_455_neural_training.py:1131 validate_reusable_complete_unit",
            "confirmed_failure_message": "training admission basin population differs from the neural unit",
            "confirmed_failure_seconds_after_last_checkpoint": 42,
            "units_completed_before_failure": 1,
            "checkpoints_written_before_failure": 30,
            "defect_1_population_whole_dict_compared_against_two_key_literal": True,
            "defect_1_could_never_pass_for_any_admission": True,
            "defect_1_also_fires_on_the_reuse_path": True,
            "defect_1_fixed_in": "scripts/run_tukf09_455_neural_training.py",
            "defect_1_fix_is_stricter_now_also_verifies_the_compact_digest": True,
            "defect_2_installed_identities_carried_the_superseded_authorization": True,
            "defect_2_resolved_by_re_migration_not_by_editing_the_seal": True,
            "selection_seal_and_independent_verifier_unedited": True,
            "re_migration_scientific_payload_unchanged": True,
            "re_migration_identical_files": ["history.npz", "selection.json", "access_ledger.json"],
            "re_migration_changed_files_per_basin": ["identity.json", "manifest.sha256.json", "migration_provenance.json"],
            "collateral_repair_shared_module_restored_for_the_original_experiment": True,
            "original_experiment_admission_reverifies": True,
            "all_scope_authorization_re_registered": False,
            "all_scope_authorization_sha256_unchanged": "4e7b3078bbdbdeabaa29c9ff0e42f3a179a643673ca592007f712b9a84b85bb6",
            "scientific_contract_changed": False,
            "population_periods_model_seeds_budget_mask_metric_threshold_changed": False,
            "exclusive_node_directive_removed": True,
            "reason_exclusive_removed": "the_colleague_objected_to_whole_node_occupancy_and_holding_all_eight_gres_already_denies_every_other_gpu_job_so_the_whole_node_compute_process_gate_passes_by_construction",
            "whole_node_compute_process_gate_weakened": False,
            "gpus": 8,
            "cpus_per_task": 32,
            "cpus_returned_to_other_users": 32,
            "excluded_nodes": [],
            "neural_model_parallelism": 8,
            "neural_units_completed": 0,
            "frozen_test_suite_passed_count": 328,
            "frozen_test_suite_skipped_count": 0,
            "preserve_superseded_remote_roots": True,
            "preserve_superseded_capsules": True,
            "same_root_training_resubmission_allowed": False,
            "formal_evaluation_authorized": False,
        }
        or source_capsule
        != {
            "schema_version": "tukf09_455_training_source_capsule_v2",
            "root": "/data1/home/sunyiq/kalmannet_tukf09_455_basin_zero_validation_target_variance_revision_v1_training_source_capsule_v6_20260908",
            "data_root": "/data1/home/sunyiq/kalmannet_tukf09_455_basin_zero_validation_target_variance_revision_v1_training_source_capsule_v6_20260908/data/camels_us",
            "manifest": "/data1/home/sunyiq/kalmannet_tukf09_455_basin_zero_validation_target_variance_revision_v1_training_source_capsule_v6_20260908/evidence/source_capsule_manifest.json",
            "manifest_size": 541788,
            "manifest_sha256": "b15936c7a6375fe003692024ff6ea63658db32fe650fb2b2c26c7ed66cb3579b",
            "manifest_sha256_record": "/data1/home/sunyiq/kalmannet_tukf09_455_basin_zero_validation_target_variance_revision_v1_training_source_capsule_v6_20260908/evidence/source_capsule_manifest.sha256",
            "manifest_sha256_record_size": 95,
            "manifest_sha256_record_sha256": "4a53a820efab42cdba0cf9ce06f813b5f7d34636b5b6398f527dd2c4bfe9af65",
            "ready": "/data1/home/sunyiq/kalmannet_tukf09_455_basin_zero_validation_target_variance_revision_v1_training_source_capsule_v6_20260908/evidence/READY.json",
            "ready_size": 1217,
            "ready_sha256": "e63e3343898ed5c16eeff0247cc63728c3ff033d013658d45c4747e53636799a",
            "data_file_count": 911,
            "evidence_file_count": 3,
            "total_file_count": 914,
            "directory_count": 44,
            "data_total_bytes": 464792200,
            "data_identity_sha256": "dd238eebc1696f73f9eee7adf924913ff5a912c8f795f8998255e87408b760da",
            "deployment_mailbox_sequence": 139,
            "deployment_command_sha256": "90fc232ee11d2583f748efaf18b92bbf7484eda43704aa3de0b27715347b7f54",
            "deployment_result_commit": "4c42adbf00dee25336a8d7a0ffdd808709fffbed",
            "deployment_result_size": 4906,
            "deployment_result_sha256": "282f4ac06ab8d39c0a8d93aadc64256f0fcaee83bdfdc433b3bd2c90658f571a",
            "failed_post_publication_audit_mailbox_sequence": 140,
            "failed_post_publication_audit_root_cause": "the_auditor_read_the_data_staging_block_from_a_configuration_of_a_version_that_is_not_deployed_yet",
            "post_publication_audit_mailbox_sequence": 141,
            "post_publication_audit_command_size": 9219,
            "post_publication_audit_command_sha256": "4609c397c3585f9c60646c791e05639cdf35fa970f4c7eb03dcdd9a996d913ad",
            "post_publication_audit_result_commit": "d734b366385345b052333ba2f49d0294afb236c3",
            "post_publication_audit_result_size": 911,
            "post_publication_audit_result_sha256": "381e8439bf9586d1995c32d6b8659458df66bc4f03b20efd901a2e8f6fcd65e2",
            "post_publication_audit_status": "PASS",
            "post_publication_audit_rehashed_files": 911,
            "post_publication_audit_rehash_failures": 0,
            "supersedes_capsule_root": "/data1/home/sunyiq/kalmannet_tukf09_455_basin_zero_validation_target_variance_revision_v1_training_source_capsule_v5_20260904",
            "supersedes_reason": "the_filter_migration_was_re_run_so_the_455_unit_identities_carry_the_authorization_now_in_force_and_the_training_admission_was_signed_again_after_the_population_check_that_ended_job_220663_was_repaired",
            "manifest_supersedes_reason_sentence_is_stale": "the_sentence_baked_into_the_capsule_manifest_was_inherited_from_the_previous_round_and_says_the_authorization_was_registered_again_which_did_not_happen_this_round_the_data_and_every_hash_are_correct",
            "data_bytes_identical_to_superseded_capsule": True,
            "publication_mode": "exclusive_root_reservation_then_exact_ready_json_then_root_mode_0555_final_gate",
            "required_root_mode": "0555",
            "required_directory_mode": "0555",
            "required_file_mode": "0444",
            "symbolic_links_allowed": False,
            "hard_link_count_above_one_allowed": False,
            "verify_manifest_ready_tree_surface_and_all_911_files_before_copy": True,
            "formal_evaluation_array_reads": 0,
            "formal_evaluation_predictions": 0,
            "formal_evaluation_metrics": 0,
            "formal_evaluation_outputs": 0,
        }
        or hpc_remote_layout.get("read_only_camels_root")
        != source_capsule.get("data_root")
        or hpc_remote_layout.get("preparation_failure_marker")
        != "/data1/home/sunyiq/kalmannet_tukf09_455_basin_zero_validation_target_variance_revision_v1_a800_exclusive_v2r12_20260908/status/PREPARATION_FAILED.json"
        or hpc_remote_layout.get("runtime_pending_glob")
        != "/data1/home/sunyiq/kalmannet_tukf09_455_basin_zero_validation_target_variance_revision_v1_a800_exclusive_v2r12_20260908/runtime_v2r12.pending.*"
        or hpc_remote_layout.get("same_root_retry_after_preparation_failure_allowed")
        is not False
        or hpc_data_staging.get("new_tree_publication")
        != "exclusive_destination_directory_reservation_then_ordinary_byte_copy_and_exact_verification"
        or hpc_data_staging.get("failed_pending_policy")
        != "preserve_the_partial_reserved_destination_and_freeze_the_v2r12_remote_root_no_same_root_retry"
        or hpc_data_staging.get("existing_tree_policy")
        != "exact_verify_only_no_overwrite"
        or offline_inputs.get("schema_version")
        != "tukf09_455_hpc_offline_runtime_inputs_a800_exclusive_v2r12"
        or offline_inputs.get("acquisition_host_shortname") != "login4"
        or offline_inputs.get("required_bootstrap_python_executable")
        != "/data1/home/sunyiq/miniconda3/envs/nh_final/bin/python"
        or offline_inputs.get("runtime_binary_lock_sha256")
        != "8bfc922ce4165eb7793f0b4f1afe9a185644858875af6a3bfd21992a23046d9f"
        or offline_inputs.get("psutil_source_lock_sha256")
        != "c021239b1cdeafff41591adec793c79820ad66ec5418dba2570ea8ff2ae60d68"
        or offline_inputs.get("locked_binary_wheel_count") != 23
        or offline_inputs.get("source_archive_count") != 1
        or offline_inputs.get("total_file_count") != 24
        or offline_inputs.get("total_bytes") != 2_817_756_909
        or offline_inputs.get("download_only_no_build_no_install") is not True
        or offline_inputs.get("compute_node_network_access_required") is not False
        or offline_inputs.get("shared_nh_final_modification_allowed") is not False
        or offline_inputs.get("compute_copy_mode")
        != "python_exclusive_buffered_byte_copy_with_fsync_no_symbolic_links_no_hard_links"
        or not isinstance(offline_records, Mapping)
        or len(offline_records) != 24
        or len(offline_binary_records) != 23
        or len(offline_source_records) != 1
        or len(offline_values) != 24
        or any(
        ...
        or sum(int(record["size"]) for record in offline_values) != 2_817_756_909
        or {record.get("sha256") for record in offline_binary_records}
        != binary_lock_hashes
        or {record.get("sha256") for record in offline_source_records}
        != source_lock_hashes
        or hpc_slurm.get("partition") != "hgpu8"
        or hpc_slurm.get("nodes") != 1
        or hpc_slurm.get("tasks") != 1
        or hpc_slurm.get("cpus_per_task") != 32
        or hpc_slurm.get("gpus") != 8
        or hpc_slurm.get("exclusive_node") is not False
        or hpc_slurm.get(
            "all_gres_held_so_no_other_gpu_job_can_be_scheduled_on_this_node"
        )
        is not True
        or hpc_slurm.get("cpus_returned_to_other_users") != 32
        or hpc_slurm.get("excluded_nodes") != []
        or hpc_slurm.get("wall_time") != "48:00:00"
        or hpc_slurm.get("memory_directive_forbidden") is not True
        or hpc_slurm.get("python_must_run_without_srun") is not True
        or hpc_slurm.get("omp_num_threads_per_neural_unit") != 4
        or hpc_slurm.get("whole_node_now_fully_used") is not True
        or hpc_runtime.get("python_version") != "3.11.13"
        or hpc_runtime.get("numpy_version") != "1.26.4"
        or hpc_runtime.get("torch_base_version") != "2.2.2"
        or hpc_runtime.get("torch_full_version") != "2.2.2+cu121"
        or hpc_runtime.get("psutil_version") != "5.9.0"
        or hpc_runtime.get("torch_cuda_runtime_version") != "12.1"
        or hpc_runtime.get("cudnn_version") != 8902
        or hpc_runtime.get("cuda_available") is not True
        or hpc_runtime.get("cuda_device_count") != 8
        or hpc_runtime.get("cuda_device_name") != "NVIDIA A800-SXM4-80GB"
        or hpc_runtime.get("cuda_compute_capability") != [8, 0]
        or hpc_runtime.get("minimum_cuda_device_total_memory_bytes")
        != 80_000_000_000
        or hpc_runtime.get("require_every_slurm_visible_gpu_to_be_an_admitted_a800")
        is not True
        or hpc_runtime.get(
            "require_pytorch_first_device_uuid_to_be_one_of_the_slurm_visible_gpus"
        )
        is not True
        or "require_current_pytorch_process_gpu_uuid_to_equal_slurm_selected_gpu_uuid"
        in hpc_runtime
        or private_runtime.get("schema_version")
        != "tukf09_455_hpc_private_runtime_manifest_a800_exclusive_v2r12"
        or private_runtime.get("bootstrap_python_executable_binary_is_bound_by_sha256")
        is not True
        or not isinstance(private_distributions, Mapping)
        or len(private_distributions) != 24
        or private_runtime.get(
            "distribution_versions_must_record_the_full_private_dependency_closure"
        )
        is not True
    ...
    if (
        validate_optional_local_filter_installation
        and _path_exists(local_filter_path)
        and _sha256_file(
...
def verify_extracted_bundle(destination: Path) -> dict[str, Any]:
    ...
    return {
        "status": "TUKF09_455_A800_EXCLUSIVE_V2R12_HPC_BUNDLE_VERIFIED",
        "experiment_id": EXPERIMENT_ID,
...
def verify_runtime_bundle(destination: Path) -> dict[str, Any]:
    ...
    expected_sources = bundle_member_sources(
        root=root / "kalmannet",
        validate_optional_local_filter_installation=False,
    )
    if set(expected_sources) != set(hashes):
    ...
    return {
        "status": "TUKF09_455_A800_EXCLUSIVE_V2R12_HPC_RUNTIME_BUNDLE_VERIFIED",
        "experiment_id": EXPERIMENT_ID,
        "member_count": len(hashes),
        "admitted_executable_count": EXPECTED_ADMITTED_EXECUTABLE_COUNT,
        "admitted_test_count": EXPECTED_ADMITTED_TEST_COUNT,
...
def extract_bundle(archive_path: Path, destination: Path) -> dict[str, Any]:
    ...
    if target.parent.is_symlink() or _is_reparse_point(target.parent):
        raise RuntimeError("extraction destination parent is linked")
    # Reserve the final directory name itself.  A partial tree is deliberately
    # preserved on any later failure, which freezes the caller's experiment
    # root and prevents loss of forensic evidence.  No check-then-replace
    ...
    for name in sorted(payloads):
        ...
        if target_resolved not in resolved.parents:
            raise RuntimeError(f"archive member escapes reserved final root: {name}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build, verify, or safely extract the deterministic TUKF09 455-basin A800-exclusive HPC v2r12 bundle."
    )
