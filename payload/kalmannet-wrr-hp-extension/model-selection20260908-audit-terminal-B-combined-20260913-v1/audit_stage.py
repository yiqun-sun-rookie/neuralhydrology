...
def parse_epoch_log(data):
    """Keep nonfinite diagnostic tokens explicit while exporting strict JSON.

    Original log bytes remain in metadata_base64 with their unchanged hash.
    Only this derived epoch-log view uses tagged objects; validation scores,
    configs, failure classification and the strict certificate encoder do not.
    """
    return [json.loads(line, parse_constant=lambda token: {"nonfinite_epoch_log_value": token})
...
def audit_evidence(evidence):
    ...
    except (ValueError, KeyError, TypeError, IndexError, SyntaxError, InvalidOperation) as exc:
...
def verify_anchors(e):
    ...
    for task in e["tasks"].values():
        ...
        if str(epoch_path) in raw:
            parsed(epoch_path, task["epochs"], parse_epoch_log)
        else:
            require(task["epochs"] == [], "Unretained completed epoch log")
...
class SchedulerSnapshotError(RuntimeError):
    """Fail closed while retaining every attempted scheduler query for stdout."""

    def __init__(self, message, status, commands):
        super().__init__(message)
        self.status = status
        self.scheduler_queries = copy.deepcopy(commands)
...
def scheduler_snapshot(parent, run=subprocess.run):
    ...
    def captured_text(value):
        # TimeoutExpired may retain bytes even when subprocess.run uses text=True.
        if isinstance(value, bytes):
            return value.decode("utf-8", errors="replace")
        return "" if value is None else value

    def fail(message, status="STOP_INTEGRITY"):
        raise SchedulerSnapshotError(message, status, commands)
    ...
    def call(args):
        try:
            ...
            if isinstance(exc, subprocess.TimeoutExpired):
                detail["timeout_seconds"] = exc.timeout
            commands.append({"command": args, "returncode": getattr(exc, "returncode", None),
                             "stdout": captured_text(getattr(exc, "stdout", None)),
                             "stderr": captured_text(getattr(exc, "stderr", None)), "exception": detail})
            raise SchedulerSnapshotError("Read-only scheduler query could not complete",
                                         "STOP_INFRASTRUCTURE", commands) from exc
        commands.append({"command": args, "returncode": response.returncode, "stdout": response.stdout, "stderr": response.stderr})
        if response.returncode != 0:
            fail("Read-only scheduler query failed", "STOP_INFRASTRUCTURE")
        return response.stdout
    fields = call(["sacct", "--helpformat"])
    if not all(re.search(r"\b" + name + r"\b", fields) for name in FIELDS):
        fail("Cluster lacks required accounting mapping fields")
    # Slurm 19.05.4 rejects --array; this exact query retains JobIDRaw mapping.
    accounting = call(["sacct", "-X", "--noheader", "--parsable2", "--jobs", parent, "--format", "JobID%64,JobIDRaw%32,State%32,ExitCode,ElapsedRaw"])
    ...
    for line in queued.splitlines():
        parts = line.split("|")
        if len(parts) != 3:
            fail("Malformed queue identity/workdir")
...
def collect(stage, receipt_path, *, run=subprocess.run):
    """Preserve completed scheduler history on subsequent collection failures."""
    queries = []
    try:
        return _collect(stage, receipt_path, run=run, query_history=queries)
    except (OSError, ValueError, KeyError, subprocess.SubprocessError) as exc:
        # Keep the original exception type/classification and traceback. Query
        # failures themselves already carry history in SchedulerSnapshotError.
        exc.scheduler_queries = copy.deepcopy(queries)
        raise


def _collect(stage, receipt_path, *, run, query_history):
    ...
    require({path.name for path in data_entries} == set(expected_links), "Unexpected data entry inventory")
    # Validate the entire metadata set before any linked target is read.
    for path in data_entries:
        require(path.is_symlink(), "Unexpected non-link data entry")
        require(str(path.resolve()) == expected_links[path.name]["resolved"], "Data link target changed")
    for path in data_entries:
    ...
    for combo in combos:
        ...
        e["tasks"][str(index)] = {
            ...
            "epochs": parse_epoch_log(epoch_text) if epoch_text else [],
...
def main(argv=None):
    ...
    try:
    ...
    except (SchedulerSnapshotError, OSError, ValueError, KeyError, subprocess.SubprocessError) as exc:
        status = exc.status if isinstance(exc, SchedulerSnapshotError) else (
        ...
        print(json.dumps({"report": report, "admission_written": False,
                          "scheduler_queries": getattr(exc, "scheduler_queries", [])}, sort_keys=True))
